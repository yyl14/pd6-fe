/* eslint-disable no-undef */
/* eslint-disable no-use-before-define */
/**
 * This file was auto-generated by openapi-typescript.
 * Do not make direct changes to the file.
 */

export interface paths {
  '/': {
    /** Default Page */
    get: operations['default_page__get'];
  };
  '/email-verification': {
    /** Email Verification */
    get: operations['email_verification_email_verification_get'];
    /** Email Verification */
    post: operations['email_verification_email_verification_post'];
  };
  '/account/forget-password': {
    /** Forget Password */
    post: operations['forget_password_account_forget_password_post'];
  };
  '/account/forget-username': {
    /** Forget Username */
    post: operations['forget_username_account_forget_username_post'];
  };
  '/s3-file/{s3_file_uuid}/url': {
    /**
     * Get S3 File Url
     * @description ### 權限
     * - SN
     *
     * ### Note
     * - 目前所有 url 都有時間限制 (超時會自動過期)
     */
    get: operations['get_s3_file_url_s3_file__s3_file_uuid__url_get'];
  };
  '/institute': {
    /**
     * Browse All Institute
     * @description ### 權限
     * - Public
     */
    get: operations['browse_all_institute_institute_get'];
    /**
     * Add Institute
     * @description ### 權限
     * - System Manager
     */
    post: operations['add_institute_institute_post'];
  };
  '/institute/{institute_id}': {
    /**
     * Read Institute
     * @description ### 權限
     * - Public
     */
    get: operations['read_institute_institute__institute_id__get'];
    /**
     * Edit Institute
     * @description ### 權限
     * - System Manager
     */
    patch: operations['edit_institute_institute__institute_id__patch'];
  };
  '/account': {
    /**
     * Browse Account With Default Student Id
     * @description ### 權限
     * - System Manager
     * {'id': 'int', 'username': 'str', 'nickname': 'str', 'role': 'str', 'real_name': 'str', 'alternative_email': 'str'}
     */
    get: operations['browse_account_with_default_student_id_account_get'];
    /** Add Account */
    post: operations['add_account_account_post'];
  };
  '/account-summary/batch': {
    /**
     * Batch Get Account With Default Student Id
     * @description ### 權限
     * - System Normal
     *
     * ### Notes
     * - `account_ids`: list of int
     */
    get: operations['batch_get_account_with_default_student_id_account_summary_batch_get'];
  };
  '/account-summary/batch-by-account-referral': {
    /**
     * Batch Get Account By Account Referrals
     * @description ### 權限
     * - System Normal
     *
     * ### Notes:
     * account_referrals: list of string
     */
    get: operations['batch_get_account_by_account_referrals_account_summary_batch_by_account_referral_get'];
  };
  '/account/{account_id}/class': {
    /**
     * Browse All Account With Class Role
     * @description ### 權限
     * - Self
     *
     * ### Available columns
     */
    get: operations['browse_all_account_with_class_role_account__account_id__class_get'];
  };
  '/account/template': {
    /**
     * Get Account Template File
     * @description ### 權限
     * - System Manager
     */
    get: operations['get_account_template_file_account_template_get'];
  };
  '/account/{account_id}': {
    /**
     * Read Account With Default Student Id
     * @description ### 權限
     * - System Manager
     * - Self
     * - System Normal (個資除外)
     */
    get: operations['read_account_with_default_student_id_account__account_id__get'];
    /**
     * Delete Account
     * @description ### 權限
     * - System manager
     * - Self
     */
    delete: operations['delete_account_account__account_id__delete'];
    /**
     * Edit Account
     * @description ### 權限
     * - System Manager
     * - Self
     */
    patch: operations['edit_account_account__account_id__patch'];
  };
  '/account/{account_id}/default-student-card': {
    /**
     * Make Student Card Default
     * @description ### 權限
     * - System manager
     * - Self
     */
    put: operations['make_student_card_default_account__account_id__default_student_card_put'];
  };
  '/account/{account_id}/email-verification': {
    /**
     * Browse All Account Pending Email Verification
     * @description ### 權限
     * - System manager
     * - Self
     */
    get: operations['browse_all_account_pending_email_verification_account__account_id__email_verification_get'];
  };
  '/account/{account_id}/student-card': {
    /**
     * Browse All Account Student Card
     * @description ### 權限
     * - System manager
     * - Self
     */
    get: operations['browse_all_account_student_card_account__account_id__student_card_get'];
    /**
     * Add Student Card To Account
     * @description ### 權限
     * - System manager
     * - Self
     */
    post: operations['add_student_card_to_account_account__account_id__student_card_post'];
  };
  '/student-card/{student_card_id}': {
    /**
     * Read Student Card
     * @description ### 權限
     * - System manager
     * - Self
     */
    get: operations['read_student_card_student_card__student_card_id__get'];
  };
  '/email-verification/{email_verification_id}/resend': {
    /**
     * Resend Email Verification
     * @description ### 權限
     * - System manager
     * - Self
     */
    post: operations['resend_email_verification_email_verification__email_verification_id__resend_post'];
  };
  '/email-verification/{email_verification_id}': {
    /**
     * Delete Pending Email Verification
     * @description ### 權限
     * - System manager
     * - Self
     */
    delete: operations['delete_pending_email_verification_email_verification__email_verification_id__delete'];
  };
  '/course': {
    /**
     * Browse All Course
     * @description ### 權限
     * - System manager (hidden)
     * - System normal (not hidden)
     */
    get: operations['browse_all_course_course_get'];
    /**
     * Add Course
     * @description ### 權限
     * - System manager
     */
    post: operations['add_course_course_post'];
  };
  '/course/{course_id}': {
    /**
     * Read Course
     * @description ### 權限
     * - System manager (hidden)
     * - System normal (not hidden)
     */
    get: operations['read_course_course__course_id__get'];
    /**
     * Delete Course
     * @description ### 權限
     * - System manager
     */
    delete: operations['delete_course_course__course_id__delete'];
    /**
     * Edit Course
     * @description ### 權限
     * - System manager
     */
    patch: operations['edit_course_course__course_id__patch'];
  };
  '/course/{course_id}/class': {
    /**
     * Browse All Class Under Course
     * @description ### 權限
     * - Class+ manager (hidden)
     * - System normal (not hidden)
     */
    get: operations['browse_all_class_under_course_course__course_id__class_get'];
    /**
     * Add Class Under Course
     * @description ### 權限
     * - System manager
     */
    post: operations['add_class_under_course_course__course_id__class_post'];
  };
  '/class': {
    /**
     * Browse Class
     * @description ### 權限
     * - system normal: all
     *
     * ### Available columns
     * {'id': 'int', 'name': 'str', 'course_id': 'int', 'is_deleted': 'bool'}
     */
    get: operations['browse_class_class_get'];
  };
  '/class/{class_id}': {
    /**
     * Read Class
     * @description ### 權限
     * - System normal: all
     */
    get: operations['read_class_class__class_id__get'];
    /**
     * Delete Class
     * @description ### 權限
     * - System manager
     */
    delete: operations['delete_class_class__class_id__delete'];
    /**
     * Edit Class
     * @description ### 權限
     * - Class+ manager
     */
    patch: operations['edit_class_class__class_id__patch'];
  };
  '/class/{class_id}/member': {
    /**
     * Browse Class Member
     * @description ### 權限
     * - Class normal
     * - Class+ manager
     *
     * ### Available columns
     * {'member_id': 'int', 'class_id': 'int', 'role': 'RoleType'}
     */
    get: operations['browse_class_member_class__class_id__member_get'];
    /**
     * Replace Class Members
     * @description ### 權限
     * - Class+ manager
     */
    put: operations['replace_class_members_class__class_id__member_put'];
  };
  '/class/{class_id}/member/account-referral': {
    /**
     * Browse All Class Member With Account Referral
     * @description ### 權限
     * - Class normal
     * - Class+ manager
     */
    get: operations['browse_all_class_member_with_account_referral_class__class_id__member_account_referral_get'];
  };
  '/class/{class_id}/member/{member_id}': {
    /**
     * Delete Class Member
     * @description ### 權限
     * - Class+ manager
     */
    delete: operations['delete_class_member_class__class_id__member__member_id__delete'];
  };
  '/class/{class_id}/team': {
    /**
     * Browse Team Under Class
     * @description ### 權限
     * - Class normal: all
     *
     * ### Available columns
     * {'id': 'int', 'name': 'str', 'class_id': 'int', 'label': 'str', 'is_deleted': 'bool'}
     */
    get: operations['browse_team_under_class_class__class_id__team_get'];
    /**
     * Add Team Under Class
     * @description ### 權限
     * - Class manager
     */
    post: operations['add_team_under_class_class__class_id__team_post'];
  };
  '/class/{class_id}/submission': {
    /**
     * Browse Submission Under Class
     * @description ### 權限
     * - Class manager
     *
     * ### Available columns
     * {'id': 'int', 'account_id': 'int', 'problem_id': 'int', 'language_id': 'int', 'content_file_uuid': 'UUID', 'content_length': 'int', 'filename': 'str', 'submit_time': 'ServerTZDatetime'}
     */
    get: operations['browse_submission_under_class_class__class_id__submission_get'];
  };
  '/class/{class_id}/team-import': {
    /**
     * Import Team
     * @description ### 權限
     * - Class manager
     */
    post: operations['import_team_class__class_id__team_import_post'];
  };
  '/team/template': {
    /**
     * Get Team Template File
     * @description ### 權限
     * - system normal
     */
    get: operations['get_team_template_file_team_template_get'];
  };
  '/team/{team_id}': {
    /**
     * Read Team
     * @description ### 權限
     * - Class normal
     */
    get: operations['read_team_team__team_id__get'];
    /**
     * Delete Team
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_team_team__team_id__delete'];
    /**
     * Edit Team
     * @description ### 權限
     * - Class manager
     * - Team manager (limited)
     */
    patch: operations['edit_team_team__team_id__patch'];
  };
  '/team/{team_id}/member': {
    /**
     * Browse Team All Member
     * @description ### 權限
     * - Class normal
     */
    get: operations['browse_team_all_member_team__team_id__member_get'];
    /**
     * Add Team Member
     * @description ### 權限
     * - class manager
     */
    post: operations['add_team_member_team__team_id__member_post'];
    /**
     * Edit Team Member
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_team_member_team__team_id__member_patch'];
  };
  '/team/{team_id}/member/{member_id}': {
    /**
     * Delete Team Member
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_team_member_team__team_id__member__member_id__delete'];
  };
  '/class/{class_id}/grade-import': {
    /**
     * Import Class Grade
     * @description ### 權限
     * - Class manager
     */
    post: operations['import_class_grade_class__class_id__grade_import_post'];
  };
  '/class/{class_id}/grade': {
    /**
     * Browse Class Grade
     * @description ### 權限
     * - Class manager (all)
     * - Class normal (self)
     *
     * ### Available columns
     * {'receiver_id': 'int', 'grader_id': 'int', 'class_id': 'int', 'title': 'str', 'score': 'str', 'comment': 'str', 'update_time': 'str'}
     */
    get: operations['browse_class_grade_class__class_id__grade_get'];
    /**
     * Add Grade
     * @description ### 權限
     * - Class Manager
     */
    post: operations['add_grade_class__class_id__grade_post'];
  };
  '/account/{account_id}/grade': {
    /**
     * Browse Account Grade
     * @description ### 權限
     * - Self
     *
     * ### Available columns
     * {'class_id': 'int', 'title': 'str', 'score': 'str', 'comment': 'str', 'update_time': 'str'}
     */
    get: operations['browse_account_grade_account__account_id__grade_get'];
  };
  '/grade/template': {
    /**
     * Get Grade Template File
     * @description ### 權限
     * - system normal
     */
    get: operations['get_grade_template_file_grade_template_get'];
  };
  '/grade/{grade_id}': {
    /**
     * Get Grade
     * @description ### 權限
     * - Class manager (all)
     * - Class normal (self)
     */
    get: operations['get_grade_grade__grade_id__get'];
    /**
     * Delete Grade
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_grade_grade__grade_id__delete'];
    /**
     * Edit Grade
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_grade_grade__grade_id__patch'];
  };
  '/class/{class_id}/challenge': {
    /**
     * Browse Challenge Under Class
     * @description ### 權限
     * - Class manager (all)
     * - System normal (not scheduled)
     *
     * ### Available columns
     * {'id': 'int', 'class_id': 'int', 'publicize_type': 'ChallengePublicizeType', 'selection_type': 'TaskSelectionType', 'title': 'str', 'setter_id': 'int', 'description': 'str', 'start_time': 'ServerTZDatetime', 'end_time': 'ServerTZDatetime', 'is_deleted': 'bool'}
     */
    get: operations['browse_challenge_under_class_class__class_id__challenge_get'];
    /**
     * Add Challenge Under Class
     * @description ### 權限
     * - Class manager
     */
    post: operations['add_challenge_under_class_class__class_id__challenge_post'];
  };
  '/challenge/{challenge_id}': {
    /**
     * Read Challenge
     * @description ### 權限
     * - Class manager (all)
     * - Class normal & guest (after start time)
     * - System normal (after scheduled time)
     */
    get: operations['read_challenge_challenge__challenge_id__get'];
    /**
     * Delete Challenge
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_challenge_challenge__challenge_id__delete'];
    /**
     * Edit Challenge
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_challenge_challenge__challenge_id__patch'];
  };
  '/challenge/{challenge_id}/problem': {
    /**
     * Add Problem Under Challenge
     * @description ### 權限
     * - Class manager
     */
    post: operations['add_problem_under_challenge_challenge__challenge_id__problem_post'];
  };
  '/challenge/{challenge_id}/essay': {
    /**
     * Add Essay Under Challenge
     * @description ### 權限
     * - Class manager
     */
    post: operations['add_essay_under_challenge_challenge__challenge_id__essay_post'];
  };
  '/challenge/{challenge_id}/peer-review': {
    /**
     * Add Peer Review Under Challenge
     * @description ### 權限
     * - Class manager
     */
    post: operations['add_peer_review_under_challenge_challenge__challenge_id__peer_review_post'];
  };
  '/challenge/{challenge_id}/team-project-scoreboard': {
    /**
     * Add Team Project Scoreboard Under Challenge
     * @description ### 權限
     * - Class manager
     */
    post: operations['add_team_project_scoreboard_under_challenge_challenge__challenge_id__team_project_scoreboard_post'];
  };
  '/challenge/{challenge_id}/task': {
    /**
     * Browse All Task Under Challenge
     * @description ### 權限
     * - Class manager (all)
     * - Class guest (active/archived challenges)
     * - System Normal (by challenge publicize type)
     */
    get: operations['browse_all_task_under_challenge_challenge__challenge_id__task_get'];
  };
  '/challenge/{challenge_id}/task-status': {
    /**
     * Browse All Task Status Under Challenge
     * @description ### 權限
     * - Self: see self
     */
    get: operations['browse_all_task_status_under_challenge_challenge__challenge_id__task_status_get'];
  };
  '/challenge/{challenge_id}/statistics/summary': {
    /**
     * Get Challenge Statistics
     * @description ### 權限
     * - class manager
     */
    get: operations['get_challenge_statistics_challenge__challenge_id__statistics_summary_get'];
  };
  '/challenge/{challenge_id}/statistics/member-submission': {
    /**
     * Get Member Submission Statistics
     * @description ### 權限
     * - class manager
     */
    get: operations['get_member_submission_statistics_challenge__challenge_id__statistics_member_submission_get'];
  };
  '/challenge/{challenge_id}/all-submission': {
    /**
     * Download All Submissions
     * @description ### 權限
     * - class manager
     */
    post: operations['download_all_submissions_challenge__challenge_id__all_submission_post'];
  };
  '/challenge/{challenge_id}/all-plagiarism-report': {
    /**
     * Download All Plagiarism Reports
     * @description ### 權限
     * - class manager
     */
    post: operations['download_all_plagiarism_reports_challenge__challenge_id__all_plagiarism_report_post'];
  };
  '/challenge/{challenge_id}/team-contest-scoreboard': {
    /**
     * Add Team Contest Scoreboard Under Challenge
     * @description ### 權限
     * - class manager
     */
    post: operations['add_team_contest_scoreboard_under_challenge_challenge__challenge_id__team_contest_scoreboard_post'];
  };
  '/problem': {
    /**
     * Browse Problem Set
     * @description ### 權限
     * - System normal (not hidden)
     */
    get: operations['browse_problem_set_problem_get'];
  };
  '/problem/{problem_id}': {
    /**
     * Read Problem
     * @description ### 權限
     * - Class manager (hidden)
     * - System normal (not hidden)
     */
    get: operations['read_problem_problem__problem_id__get'];
    /**
     * Delete Problem
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_problem_problem__problem_id__delete'];
    /**
     * Edit Problem
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_problem_problem__problem_id__patch'];
  };
  '/problem/{problem_id}/testcase': {
    /**
     * Browse All Testcase Under Problem
     * @description ### 權限
     * - System normal (data without file uuid)
     * - CM (all data)
     */
    get: operations['browse_all_testcase_under_problem_problem__problem_id__testcase_get'];
    /**
     * Add Testcase Under Problem
     * @description ### 權限
     * - Class manager
     */
    post: operations['add_testcase_under_problem_problem__problem_id__testcase_post'];
  };
  '/problem/{problem_id}/assisting-data': {
    /**
     * Browse All Assisting Data Under Problem
     * @description ### 權限
     * - class manager
     */
    get: operations['browse_all_assisting_data_under_problem_problem__problem_id__assisting_data_get'];
    /**
     * Add Assisting Data Under Problem
     * @description ### 權限
     * - class manager
     */
    post: operations['add_assisting_data_under_problem_problem__problem_id__assisting_data_post'];
  };
  '/problem/{problem_id}/all-assisting-data': {
    /**
     * Download All Assisting Data
     * @description ### 權限
     * - class manager
     */
    post: operations['download_all_assisting_data_problem__problem_id__all_assisting_data_post'];
  };
  '/problem/{problem_id}/all-sample-testcase': {
    /**
     * Download All Sample Testcase
     * @description ### 權限
     * - class manager
     */
    post: operations['download_all_sample_testcase_problem__problem_id__all_sample_testcase_post'];
  };
  '/problem/{problem_id}/all-non-sample-testcase': {
    /**
     * Download All Non Sample Testcase
     * @description ### 權限
     * - class manager
     */
    post: operations['download_all_non_sample_testcase_problem__problem_id__all_non_sample_testcase_post'];
  };
  '/problem/{problem_id}/score': {
    /**
     * Get Score By Challenge Type Under Problem
     * @description ### 權限
     * - Self
     */
    get: operations['get_score_by_challenge_type_under_problem_problem__problem_id__score_get'];
  };
  '/problem/{problem_id}/best-score': {
    /**
     * Get Score By Best Under Problem
     * @description ### 權限
     * - Self
     */
    get: operations['get_score_by_best_under_problem_problem__problem_id__best_score_get'];
  };
  '/problem/{problem_id}/rejudge': {
    /**
     * Rejudge Problem
     * @description ### 權限
     * - Class manager
     */
    post: operations['rejudge_problem_problem__problem_id__rejudge_post'];
  };
  '/problem/{problem_id}/statistics': {
    /**
     * Get Problem Statistics
     * @description ### 權限
     * - System normal
     */
    get: operations['get_problem_statistics_problem__problem_id__statistics_get'];
  };
  '/testcase/{testcase_id}': {
    /**
     * Read Testcase
     * @description ### 權限
     * - System normal
     */
    get: operations['read_testcase_testcase__testcase_id__get'];
    /**
     * Delete Testcase
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_testcase_testcase__testcase_id__delete'];
    /**
     * Edit Testcase
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_testcase_testcase__testcase_id__patch'];
  };
  '/testcase/{testcase_id}/input-data': {
    /**
     * Upload Testcase Input Data
     * @description ### 權限
     * - Class manager
     */
    put: operations['upload_testcase_input_data_testcase__testcase_id__input_data_put'];
    /**
     * Delete Testcase Input Data
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_testcase_input_data_testcase__testcase_id__input_data_delete'];
  };
  '/testcase/{testcase_id}/output-data': {
    /**
     * Upload Testcase Output Data
     * @description ### 權限
     * - Class manager
     */
    put: operations['upload_testcase_output_data_testcase__testcase_id__output_data_put'];
    /**
     * Delete Testcase Output Data
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_testcase_output_data_testcase__testcase_id__output_data_delete'];
  };
  '/assisting-data/{assisting_data_id}': {
    /**
     * Read Assisting Data
     * @description ### 權限
     * - class manager
     */
    get: operations['read_assisting_data_assisting_data__assisting_data_id__get'];
    /**
     * Edit Assisting Data
     * @description ### 權限
     * - class manager
     */
    put: operations['edit_assisting_data_assisting_data__assisting_data_id__put'];
    /**
     * Delete Assisting Data
     * @description ### 權限
     * - class manager
     */
    delete: operations['delete_assisting_data_assisting_data__assisting_data_id__delete'];
  };
  '/submission/language': {
    /**
     * Browse All Submission Language
     * @description ### 權限
     * - System normal
     */
    get: operations['browse_all_submission_language_submission_language_get'];
    /**
     * Add Submission Language
     * @description ### 權限
     * - System manager
     */
    post: operations['add_submission_language_submission_language_post'];
  };
  '/submission/language/{language_id}': {
    /**
     * Edit Submission Language
     * @description ### 權限
     * - System manager
     */
    patch: operations['edit_submission_language_submission_language__language_id__patch'];
  };
  '/problem/{problem_id}/submission': {
    /**
     * Submit
     * @description ### 權限
     * - System Manager (all)
     * - System normal (non scheduled)
     *
     * ### 限制
     * - 上傳檔案 < 1mb
     */
    post: operations['submit_problem__problem_id__submission_post'];
  };
  '/submission': {
    /**
     * Browse Submission
     * @description ### 權限
     * - Self: see self
     *
     * ### Available columns
     * {'problem_id': 'int', 'language_id': 'int', 'submit_time': 'ServerTZDatetime'}
     */
    get: operations['browse_submission_submission_get'];
  };
  '/submission/judgment/batch': {
    /**
     * Batch Get Submission Judgment
     * @description ### 權限
     * - System Normal
     *
     * ### Notes
     * - `submission_ids`: list of int
     */
    get: operations['batch_get_submission_judgment_submission_judgment_batch_get'];
  };
  '/submission/{submission_id}': {
    /**
     * Read Submission
     * @description ### 權限
     * - Self
     * - Class manager
     */
    get: operations['read_submission_submission__submission_id__get'];
  };
  '/submission/{submission_id}/judgment': {
    /**
     * Browse All Submission Judgment
     * @description ### 權限
     * - Class manager
     */
    get: operations['browse_all_submission_judgment_submission__submission_id__judgment_get'];
  };
  '/submission/{submission_id}/latest-judgment': {
    /**
     * Read Submission Latest Judgment
     * @description ### 權限
     * - Self: see self
     * - Class manager: see class
     */
    get: operations['read_submission_latest_judgment_submission__submission_id__latest_judgment_get'];
  };
  '/submission/{submission_id}/rejudge': {
    /**
     * Rejudge Submission
     * @description ### 權限
     * - Class manager
     */
    post: operations['rejudge_submission_submission__submission_id__rejudge_post'];
  };
  '/judgment/verdict': {
    /**
     * Browse All Judgment Verdict
     * @description ### 權限
     * - Public
     */
    get: operations['browse_all_judgment_verdict_judgment_verdict_get'];
  };
  '/judgment/{judgment_id}': {
    /**
     * Read Judgment
     * @description ### 權限
     * - Self
     * - Class manager
     */
    get: operations['read_judgment_judgment__judgment_id__get'];
  };
  '/judgment/{judgment_id}/judge-case': {
    /**
     * Browse All Judgment Judge Case
     * @description ### 權限
     * - Self
     * - Class manager
     */
    get: operations['browse_all_judgment_judge_case_judgment__judgment_id__judge_case_get'];
  };
  '/essay/{essay_id}': {
    /**
     * Read Essay
     * @description ### 權限
     * - class normal
     */
    get: operations['read_essay_essay__essay_id__get'];
    /**
     * Delete Essay
     * @description ### 權限
     * - class manager
     */
    delete: operations['delete_essay_essay__essay_id__delete'];
    /**
     * Edit Essay
     * @description ### 權限
     * - class manager
     */
    patch: operations['edit_essay_essay__essay_id__patch'];
  };
  '/essay/{essay_id}/all-essay-submission': {
    /**
     * Download All Essay Submission
     * @description ### 權限
     * - class manager
     */
    post: operations['download_all_essay_submission_essay__essay_id__all_essay_submission_post'];
  };
  '/essay/{essay_id}/essay-submission': {
    /**
     * Browse Essay Submission By Essay Id
     * @description ### 權限
     * - Self
     *
     * ### Available columns
     * {'id': 'int', 'account_id': 'int', 'essay_id': 'int', 'content_file_uuid': 'UUID', 'filename': 'str', 'submit_time': 'ServerTZDatetime'}
     */
    get: operations['browse_essay_submission_by_essay_id_essay__essay_id__essay_submission_get'];
    /**
     * Upload Essay
     * @description ### 權限
     * - class normal
     *
     * ### 限制
     * - 上傳檔案 < 10mb
     */
    post: operations['upload_essay_essay__essay_id__essay_submission_post'];
  };
  '/essay-submission/{essay_submission_id}': {
    /**
     * Read Essay Submission
     * @description ### 權限
     * - class manager (always)
     * - class normal (self)
     */
    get: operations['read_essay_submission_essay_submission__essay_submission_id__get'];
    /**
     * Reupload Essay
     * @description ### 權限
     * - Class normal = self
     *
     * ### 限制
     * - 上傳檔案 < 10mb
     */
    put: operations['reupload_essay_essay_submission__essay_submission_id__put'];
  };
  '/peer-review/{peer_review_id}': {
    /**
     * Read Peer Review
     * @description ### 權限
     * - Class manager (hidden)
     * - Class normal (not hidden)
     */
    get: operations['read_peer_review_peer_review__peer_review_id__get'];
    /**
     * Delete Peer Review
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_peer_review_peer_review__peer_review_id__delete'];
    /**
     * Edit Peer Review
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_peer_review_peer_review__peer_review_id__patch'];
  };
  '/peer-review/{peer_review_id}/record': {
    /**
     * Browse Peer Review Record
     * @description ### 權限
     * - Class manager (full)
     * - Self (看不到對方)
     *
     * ### Available columns
     * {'grader_id': 'int', 'receiver_id': 'int', 'score': 'int', 'comment': 'str', 'submit_time': 'ServerTZDatetime'}
     */
    get: operations['browse_peer_review_record_peer_review__peer_review_id__record_get'];
    /**
     * Assign Peer Review Record
     * @description 發互評 (決定 A 要評誰 )
     *
     * ### 權限
     * - Self is class *normal ONLY*
     */
    post: operations['assign_peer_review_record_peer_review__peer_review_id__record_post'];
  };
  '/peer-review-record/{peer_review_record_id}': {
    /**
     * Read Peer Review Record
     * @description ### 權限
     * - Class manager (full)
     * - Self = Class normal (看不到對方)
     */
    get: operations['read_peer_review_record_peer_review_record__peer_review_record_id__get'];
    /**
     * Submit Peer Review Record
     * @description 互評完了，交互評成績評語
     *
     * ### 權限
     * - Self is class *normal ONLY*
     */
    patch: operations['submit_peer_review_record_peer_review_record__peer_review_record_id__patch'];
  };
  '/peer-review/{peer_review_id}/account/{account_id}/receive': {
    /**
     * Browse Account Received Peer Review Record
     * @description ### 權限
     * - Class Manager (all)
     * - Self = Class normal
     */
    get: operations['browse_account_received_peer_review_record_peer_review__peer_review_id__account__account_id__receive_get'];
  };
  '/peer-review/{peer_review_id}/account/{account_id}/review': {
    /**
     * Browse Account Reviewed Peer Review Record
     * @description ### 權限
     * - Class Manager (all)
     * - Self = Class normal
     */
    get: operations['browse_account_reviewed_peer_review_record_peer_review__peer_review_id__account__account_id__review_get'];
  };
  '/scoreboard/{scoreboard_id}': {
    /**
     * Read Scoreboard
     * @description ### 權限
     * - Class normal
     */
    get: operations['read_scoreboard_scoreboard__scoreboard_id__get'];
    /**
     * Delete Scoreboard
     * @description ### 權限
     * - Class manager
     */
    delete: operations['delete_scoreboard_scoreboard__scoreboard_id__delete'];
  };
  '/team-project-scoreboard/view/{scoreboard_id}': {
    /**
     * View Team Project Scoreboard
     * @description ### 權限
     * - Class normal
     */
    get: operations['view_team_project_scoreboard_team_project_scoreboard_view__scoreboard_id__get'];
  };
  '/team-project-scoreboard/{scoreboard_id}': {
    /**
     * Edit Team Project Scoreboard
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_team_project_scoreboard_team_project_scoreboard__scoreboard_id__patch'];
  };
  '/team-contest-scoreboard/view/{scoreboard_id}': {
    /**
     * View Team Contest Scoreboard
     * @description ### 權限
     * - System Normal
     */
    get: operations['view_team_contest_scoreboard_team_contest_scoreboard_view__scoreboard_id__get'];
  };
  '/team-contest-scoreboard/{scoreboard_id}': {
    /**
     * Edit Team Contest Scoreboard
     * @description ### 權限
     * - Class manager
     */
    patch: operations['edit_team_contest_scoreboard_team_contest_scoreboard__scoreboard_id__patch'];
  };
  '/announcement': {
    /**
     * Browse Announcement
     * @description ### 權限
     * - System manager (all)
     * - System guest (limited)
     *
     * ### Available columns
     * {'id': 'int', 'title': 'str', 'content': 'str', 'author_id': 'int', 'post_time': 'ServerTZDatetime', 'expire_time': 'ServerTZDatetime', 'is_deleted': 'bool'}
     */
    get: operations['browse_announcement_announcement_get'];
    /**
     * Add Announcement
     * @description ### 權限
     * - System manager
     */
    post: operations['add_announcement_announcement_post'];
  };
  '/announcement/{announcement_id}': {
    /**
     * Read Announcement
     * @description ### 權限
     * - System manager (all)
     * - System guest (limited)
     */
    get: operations['read_announcement_announcement__announcement_id__get'];
    /**
     * Delete Announcement
     * @description ### 權限
     * - System manager
     */
    delete: operations['delete_announcement_announcement__announcement_id__delete'];
    /**
     * Edit Announcement
     * @description ### 權限
     * - System manager
     */
    patch: operations['edit_announcement_announcement__announcement_id__patch'];
  };
  '/access-log': {
    /**
     * Browse Access Log
     * @description ### 權限
     * - Class+ manager
     *
     * ### Available columns
     * {'access_time': 'ServerTZDatetime', 'request_method': 'str', 'resource_path': 'str', 'ip': 'str', 'account_id': 'int'}
     */
    get: operations['browse_access_log_access_log_get'];
  };
  '/view/account': {
    /**
     * Browse Account With Default Student Id
     * @description ### 權限
     * - System Manager
     *
     * ### Available columns
     * {'account_id': 'int', 'username': 'str', 'real_name': 'str', 'student_id': 'str'}
     */
    get: operations['browse_account_with_default_student_id_view_account_get'];
  };
  '/class/{class_id}/view/member': {
    /**
     * Browse Class Member
     * @description ### 權限
     * - Class normal
     * - Class+ manager
     *
     * ### Available columns
     * {'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'abbreviated_name': 'str', 'role': 'RoleType', 'class_id': 'int'}
     */
    get: operations['browse_class_member_class__class_id__view_member_get'];
  };
  '/class/{class_id}/view/submission': {
    /**
     * Browse Submission Under Class
     * @description ### 權限
     * - Class manager
     *
     * ### Available columns
     * {'submission_id': 'int', 'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'challenge_id': 'int', 'challenge_title': 'str', 'problem_id': 'int', 'challenge_label': 'str', 'verdict': 'VerdictType', 'submit_time': 'ServerTZDatetime', 'class_id': 'int'}
     */
    get: operations['browse_submission_under_class_class__class_id__view_submission_get'];
  };
  '/view/my-submission': {
    /**
     * Browse Submission
     * @description ### 權限
     * - Self: see self
     *
     * ### Available columns
     * {'submission_id': 'int', 'course_id': 'int', 'course_name': 'str', 'class_id': 'int', 'class_name': 'str', 'challenge_id': 'int', 'challenge_title': 'str', 'problem_id': 'int', 'challenge_label': 'str', 'verdict': 'VerdictType', 'submit_time': 'ServerTZDatetime', 'account_id': 'int'}
     */
    get: operations['browse_submission_view_my_submission_get'];
  };
  '/problem/{problem_id}/view/my-submission': {
    /**
     * Browse My Submission Under Problem
     * @description ### 權限
     * - Self: see self
     *
     * ### Available columns
     * {'submission_id': 'int', 'judgment_id': 'int', 'verdict': 'VerdictType', 'score': 'int', 'total_time': 'int', 'max_memory': 'int', 'submit_time': 'ServerTZDatetime', 'account_id': 'int', 'problem_id': 'int'}
     */
    get: operations['browse_my_submission_under_problem_problem__problem_id__view_my_submission_get'];
  };
  '/class/{class_id}/view/problem-set': {
    /**
     * Browse Problem Set Under Class
     * @description ### 權限
     * - System normal (not hidden)
     *
     * ### Available columns
     * {'challenge_id': 'int', 'challenge_title': 'str', 'problem_id': 'int', 'challenge_label': 'str', 'problem_title': 'str', 'class_id': 'int'}
     */
    get: operations['browse_problem_set_under_class_class__class_id__view_problem_set_get'];
  };
  '/class/{class_id}/view/grade': {
    /**
     * Browse Class Grade
     * @description ### 權限
     * - Class manager (all)
     * - Class normal (self)
     *
     * ### Available columns
     * {'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'title': 'str', 'score': 'str', 'update_time': 'ServerTZDatetime', 'grade_id': 'int', 'class_id': 'int'}
     */
    get: operations['browse_class_grade_class__class_id__view_grade_get'];
  };
  '/view/access-log': {
    /**
     * Browse Access Log
     * @description ### 權限
     * - Class+ manager
     *
     * ### Available columns
     * {'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'ip': 'str', 'resource_path': 'str', 'request_method': 'str', 'access_time': 'ServerTZDatetime', 'access_log_id': 'int'}
     */
    get: operations['browse_access_log_view_access_log_get'];
  };
  '/peer-review/{peer_review_id}/view/reviewer-summary': {
    /**
     * Peer Review Summary Review
     * @description ### 權限
     * - Class Manager
     *
     * ### Available columns
     * {'username': 'str', 'real_name': 'str', 'student_id': 'str', 'average_score': 'float'}
     */
    get: operations['peer_review_summary_review_peer_review__peer_review_id__view_reviewer_summary_get'];
  };
  '/peer-review/{peer_review_id}/view/receiver-summary': {
    /**
     * Peer Review Summary Receive
     * @description ### 權限
     * - Class Manager
     *
     * ### Available columns
     * {'username': 'str', 'real_name': 'str', 'student_id': 'str', 'average_score': 'float'}
     */
    get: operations['peer_review_summary_receive_peer_review__peer_review_id__view_receiver_summary_get'];
  };
  '/hardcode/team-contest-scoreboard/{scoreboard_id}/runs': {
    /**
     * View Team Contest Scoreboard Runs
     * @description ### 權限
     * - System Normal
     */
    get: operations['view_team_contest_scoreboard_runs_hardcode_team_contest_scoreboard__scoreboard_id__runs_get'];
  };
  '/account/jwt': {
    /** Login */
    post: operations['login_account_jwt_post'];
  };
  '/account-normal': {
    /**
     * Add Normal Account
     * @description ### 權限
     * - System Manager
     */
    post: operations['add_normal_account_account_normal_post'];
  };
  '/account-import': {
    /**
     * Import Account
     * @description ### 權限
     * - System Manager
     */
    post: operations['import_account_account_import_post'];
  };
  '/account/{account_id}/pass_hash': {
    /**
     * Edit Password
     * @description ### 權限
     * - System Manager
     * - Self (need old password)
     */
    put: operations['edit_password_account__account_id__pass_hash_put'];
  };
  '/account/reset-password': {
    /** Reset Password */
    post: operations['reset_password_account_reset_password_post'];
  };
  '/metrics': {
    /**
     * Metrics
     * @description Endpoint that serves Prometheus metrics.
     */
    get: operations['metrics_metrics_get'];
  };
}

export type webhooks = Record<string, never>;

export interface components {
  schemas: {
    /** AddAccountInput */
    AddAccountInput: {
      /** Username */
      username: string;
      /** Password */
      password: string;
      /** Nickname */
      nickname: string;
      /** Real Name */
      real_name: string;
      /**
       * Alternative Email
       * Format: email
       */
      alternative_email?: string;
      /** Institute Id */
      institute_id: number;
      /** Student Id */
      student_id: string;
      /** Institute Email Prefix */
      institute_email_prefix: string;
    };
    /** AddAnnouncementInput */
    AddAnnouncementInput: {
      /** Title */
      title: string;
      /** Content */
      content: string;
      /** Author Id */
      author_id: number;
      /**
       * Post Time
       * Format: date-time
       */
      post_time: string;
      /**
       * Expire Time
       * Format: date-time
       */
      expire_time: string;
    };
    /** AddChallengeInput */
    AddChallengeInput: {
      publicize_type: components['schemas']['ChallengePublicizeType'];
      selection_type: components['schemas']['TaskSelectionType'];
      /** Title */
      title: string;
      /** Description */
      description?: string;
      /**
       * Start Time
       * Format: date-time
       */
      start_time: string;
      /**
       * End Time
       * Format: date-time
       */
      end_time: string;
    };
    /** AddClassInput */
    AddClassInput: {
      /** Name */
      name: string;
    };
    /** AddCourseInput */
    AddCourseInput: {
      /** Name */
      name: string;
      type: components['schemas']['CourseType'];
    };
    /** AddEssayInput */
    AddEssayInput: {
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Description */
      description?: string;
    };
    /** AddGradeInput */
    AddGradeInput: {
      /** Receiver Referral */
      receiver_referral: string;
      /** Grader Referral */
      grader_referral: string;
      /** Title */
      title: string;
      /** Score */
      score: string;
      /** Comment */
      comment: string;
    };
    /** AddInstituteInput */
    AddInstituteInput: {
      /** Abbreviated Name */
      abbreviated_name: string;
      /** Full Name */
      full_name: string;
      /** Email Domain */
      email_domain: string;
      /** Is Disabled */
      is_disabled: boolean;
    };
    /** AddMemberInput */
    AddMemberInput: {
      /** Account Referral */
      account_referral: string;
      role: components['schemas']['RoleType'];
    };
    /** AddNormalAccountInput */
    AddNormalAccountInput: {
      /** Real Name */
      real_name: string;
      /** Username */
      username: string;
      /** Password */
      password: string;
      /**
       * Nickname
       * @default
       */
      nickname?: string;
      /**
       * Alternative Email
       * Format: email
       */
      alternative_email?: string;
    };
    /** AddPeerReviewInput */
    AddPeerReviewInput: {
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Target Problem Id */
      target_problem_id: number;
      /** Description */
      description: string;
      /** Min Score */
      min_score: number;
      /** Max Score */
      max_score: number;
      /** Max Review Count */
      max_review_count: number;
    };
    /** AddProblemInput */
    AddProblemInput: {
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Full Score */
      full_score?: number;
      /** Description */
      description?: string;
      /** Io Description */
      io_description?: string;
      /** Source */
      source?: string;
      /** Hint */
      hint?: string;
    };
    /** AddStudentCardInput */
    AddStudentCardInput: {
      /** Institute Id */
      institute_id: number;
      /** Institute Email Prefix */
      institute_email_prefix: string;
      /** Student Id */
      student_id: string;
    };
    /** AddSubmissionLanguageInput */
    AddSubmissionLanguageInput: {
      /** Name */
      name: string;
      /** Version */
      version: string;
      /** Queue Name */
      queue_name: string;
      /** Is Disabled */
      is_disabled: boolean;
    };
    /** AddTeamContestScoreboardInput */
    AddTeamContestScoreboardInput: {
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Target Problem Ids */
      target_problem_ids: number[];
      /** Penalty Formula */
      penalty_formula: string;
      /** Team Label Filter */
      team_label_filter?: string;
    };
    /** AddTeamInput */
    AddTeamInput: {
      /** Name */
      name: string;
      /** Label */
      label: string;
    };
    /** AddTeamProjectScoreboardInput */
    AddTeamProjectScoreboardInput: {
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Target Problem Ids */
      target_problem_ids: number[];
      /** Scoring Formula */
      scoring_formula: string;
      /** Baseline Team Id */
      baseline_team_id?: number;
      /** Rank By Total Score */
      rank_by_total_score: boolean;
      /** Team Label Filter */
      team_label_filter?: string;
    };
    /** AddTestcaseInput */
    AddTestcaseInput: {
      /** Is Sample */
      is_sample: boolean;
      /** Score */
      score: number;
      /** Time Limit */
      time_limit: number;
      /** Memory Limit */
      memory_limit: number;
      /** Note */
      note?: string;
      /** Is Disabled */
      is_disabled: boolean;
      /** Label */
      label: string;
    };
    /** Announcement */
    Announcement: {
      /** Id */
      id: number;
      /** Title */
      title: string;
      /** Content */
      content: string;
      /** Author Id */
      author_id: number;
      /**
       * Post Time
       * Format: date-time
       */
      post_time: string;
      /**
       * Expire Time
       * Format: date-time
       */
      expire_time: string;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** AssignPeerReviewOutput */
    AssignPeerReviewOutput: {
      /** Id */
      id: number[];
    };
    /** Body_add_assisting_data_under_problem_problem__problem_id__assisting_data_post */
    Body_add_assisting_data_under_problem_problem__problem_id__assisting_data_post: {
      /**
       * Assisting Data
       * Format: binary
       */
      assisting_data: string;
    };
    /** Body_edit_assisting_data_assisting_data__assisting_data_id__put */
    Body_edit_assisting_data_assisting_data__assisting_data_id__put: {
      /**
       * Assisting Data File
       * Format: binary
       */
      assisting_data_file: string;
    };
    /** Body_import_account_account_import_post */
    Body_import_account_account_import_post: {
      /**
       * Account File
       * Format: binary
       */
      account_file: string;
    };
    /** Body_import_class_grade_class__class_id__grade_import_post */
    Body_import_class_grade_class__class_id__grade_import_post: {
      /**
       * Grade File
       * Format: binary
       */
      grade_file: string;
    };
    /** Body_import_team_class__class_id__team_import_post */
    Body_import_team_class__class_id__team_import_post: {
      /**
       * Team File
       * Format: binary
       */
      team_file: string;
    };
    /** Body_reupload_essay_essay_submission__essay_submission_id__put */
    Body_reupload_essay_essay_submission__essay_submission_id__put: {
      /**
       * Essay File
       * Format: binary
       */
      essay_file: string;
    };
    /** Body_submit_problem__problem_id__submission_post */
    Body_submit_problem__problem_id__submission_post: {
      /**
       * Content File
       * Format: binary
       */
      content_file: string;
    };
    /** Body_upload_essay_essay__essay_id__essay_submission_post */
    Body_upload_essay_essay__essay_id__essay_submission_post: {
      /**
       * Essay File
       * Format: binary
       */
      essay_file: string;
    };
    /** Body_upload_testcase_input_data_testcase__testcase_id__input_data_put */
    Body_upload_testcase_input_data_testcase__testcase_id__input_data_put: {
      /**
       * Input File
       * Format: binary
       */
      input_file: string;
    };
    /** Body_upload_testcase_output_data_testcase__testcase_id__output_data_put */
    Body_upload_testcase_output_data_testcase__testcase_id__output_data_put: {
      /**
       * Output File
       * Format: binary
       */
      output_file: string;
    };
    /** BrowseAccountWithRoleOutput */
    BrowseAccountWithRoleOutput: {
      /** Member Id */
      member_id: number;
      role: components['schemas']['RoleType'];
      /** Class Id */
      class_id: number;
      /** Class Name */
      class_name: string;
      /** Course Id */
      course_id: number;
      /** Course Name */
      course_name: string;
    };
    /** BrowseTaskOutput */
    BrowseTaskOutput: {
      /** Problem */
      problem: components['schemas']['pydantic__dataclasses__Problem'][];
      /** Peer Review */
      peer_review: components['schemas']['pydantic__dataclasses__PeerReview'][];
      /** Essay */
      essay: components['schemas']['pydantic__dataclasses__Essay'][];
      /** Scoreboard */
      scoreboard: components['schemas']['Scoreboard'][];
    };
    /** Challenge */
    Challenge: {
      /** Id */
      id: number;
      /** Class Id */
      class_id: number;
      publicize_type: components['schemas']['ChallengePublicizeType'];
      selection_type: components['schemas']['TaskSelectionType'];
      /** Title */
      title: string;
      /** Setter Id */
      setter_id: number;
      /** Description */
      description: string;
      /**
       * Start Time
       * Format: date-time
       */
      start_time: string;
      /**
       * End Time
       * Format: date-time
       */
      end_time: string;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /**
     * ChallengePublicizeType
     * @description An enumeration.
     * @enum {string}
     */
    ChallengePublicizeType: 'START_TIME' | 'END_TIME';
    /**
     * CourseType
     * @description An enumeration.
     * @enum {string}
     */
    CourseType: 'LESSON' | 'CONTEST';
    /** CustomizedReviserInput */
    CustomizedReviserInput: {
      /** Judge Language */
      judge_language: string;
      /** Judge Code */
      judge_code: string;
    };
    /** DefaultStudentCardInput */
    DefaultStudentCardInput: {
      /** Student Card Id */
      student_card_id: number;
    };
    /** EditAccountInput */
    EditAccountInput: {
      /** Username */
      username?: string;
      /** Nickname */
      nickname?: string;
      /**
       * Alternative Email
       * Format: email
       */
      alternative_email?: string;
      /** Real Name */
      real_name?: string;
    };
    /** EditAnnouncementInput */
    EditAnnouncementInput: {
      /** Title */
      title?: string;
      /** Content */
      content?: string;
      /**
       * Post Time
       * Format: date-time
       */
      post_time?: string;
      /**
       * Expire Time
       * Format: date-time
       */
      expire_time?: string;
    };
    /** EditChallengeInput */
    EditChallengeInput: {
      publicize_type?: components['schemas']['ChallengePublicizeType'];
      selection_type?: components['schemas']['TaskSelectionType'];
      /** Title */
      title?: string;
      /** Description */
      description?: string;
      /**
       * Start Time
       * Format: date-time
       */
      start_time?: string;
      /**
       * End Time
       * Format: date-time
       */
      end_time?: string;
    };
    /** EditClassInput */
    EditClassInput: {
      /** Name */
      name?: string;
      /** Course Id */
      course_id?: number;
    };
    /** EditCourseInput */
    EditCourseInput: {
      /** Name */
      name?: string;
      type?: components['schemas']['CourseType'];
    };
    /** EditEssayInput */
    EditEssayInput: {
      /** Title */
      title?: string;
      /** Challenge Label */
      challenge_label?: string;
      /** Description */
      description?: string;
    };
    /** EditGradeInput */
    EditGradeInput: {
      /** Title */
      title?: string;
      /** Score */
      score?: string;
      /** Comment */
      comment?: string;
    };
    /** EditInstituteInput */
    EditInstituteInput: {
      /** Abbreviated Name */
      abbreviated_name?: string;
      /** Full Name */
      full_name?: string;
      /** Email Domain */
      email_domain?: string;
      /** Is Disabled */
      is_disabled?: boolean;
    };
    /** EditMemberInput */
    EditMemberInput: {
      /** Member Id */
      member_id: number;
      role: components['schemas']['RoleType'];
    };
    /** EditPasswordInput */
    EditPasswordInput: {
      /** Old Password */
      old_password?: string;
      /** New Password */
      new_password: string;
    };
    /** EditPeerReviewInput */
    EditPeerReviewInput: {
      /** Challenge Label */
      challenge_label?: string;
      /** Title */
      title?: string;
      /** Description */
      description?: string;
      /** Min Score */
      min_score?: number;
      /** Max Score */
      max_score?: number;
      /** Max Review Count */
      max_review_count?: number;
    };
    /** EditProblemInput */
    EditProblemInput: {
      /** Challenge Label */
      challenge_label?: string;
      /** Title */
      title?: string;
      /** Full Score */
      full_score?: number;
      /** Testcase Disabled */
      testcase_disabled?: boolean;
      /** Description */
      description?: string;
      /** Io Description */
      io_description?: string;
      /** Source */
      source?: string;
      /** Hint */
      hint?: string;
      judge_type: components['schemas']['ProblemJudgeType'];
      judge_source?: components['schemas']['JudgeSourceInput'];
      /** Reviser Is Enabled */
      reviser_is_enabled?: boolean;
      reviser?: components['schemas']['CustomizedReviserInput'];
      /** Is Lazy Judge */
      is_lazy_judge?: boolean;
    };
    /** EditSubmissionLanguageInput */
    EditSubmissionLanguageInput: {
      /** Name */
      name?: string;
      /** Version */
      version?: string;
      /** Is Disabled */
      is_disabled?: boolean;
    };
    /** EditTeamInput */
    EditTeamInput: {
      /** Name */
      name?: string;
      /** Class Id */
      class_id?: number;
      /** Label */
      label?: string;
    };
    /** EditTestcaseInput */
    EditTestcaseInput: {
      /** Is Sample */
      is_sample?: boolean;
      /** Score */
      score?: number;
      /** Time Limit */
      time_limit?: number;
      /** Memory Limit */
      memory_limit?: number;
      /** Note */
      note?: string;
      /** Is Disabled */
      is_disabled?: boolean;
      /** Label */
      label?: string;
    };
    /** EmailVerification */
    EmailVerification: {
      /** Id */
      id: number;
      /** Email */
      email: string;
      /** Account Id */
      account_id: number;
      /** Institute Id */
      institute_id: number;
      /** Student Id */
      student_id: string;
      /** Is Consumed */
      is_consumed: boolean;
    };
    /** EssaySubmission */
    EssaySubmission: {
      /** Id */
      id: number;
      /** Account Id */
      account_id: number;
      /** Essay Id */
      essay_id: number;
      /**
       * Content File Uuid
       * Format: uuid
       */
      content_file_uuid: string;
      /** Filename */
      filename: string;
      /**
       * Submit Time
       * Format: date-time
       */
      submit_time: string;
    };
    /** ForgetPasswordInput */
    ForgetPasswordInput: {
      /** Username */
      username: string;
      /**
       * Email
       * Format: email
       */
      email: string;
    };
    /** ForgetUsernameInput */
    ForgetUsernameInput: {
      /**
       * Email
       * Format: email
       */
      email: string;
    };
    /** GetAccountTemplateOutput */
    GetAccountTemplateOutput: {
      /**
       * S3 File Uuid
       * Format: uuid
       */
      s3_file_uuid: string;
      /** Filename */
      filename: string;
    };
    /** GetChallengeStatOutput */
    GetChallengeStatOutput: {
      /** Tasks */
      tasks: components['schemas']['ProblemStatOutput'][];
    };
    /** GetGradeTemplateOutput */
    GetGradeTemplateOutput: {
      /**
       * S3 File Uuid
       * Format: uuid
       */
      s3_file_uuid: string;
      /** Filename */
      filename: string;
    };
    /** GetProblemStatOutput */
    GetProblemStatOutput: {
      /** Solved Member Count */
      solved_member_count: number;
      /** Submission Count */
      submission_count: number;
      /** Member Count */
      member_count: number;
    };
    /** GetTeamTemplateOutput */
    GetTeamTemplateOutput: {
      /**
       * S3 File Uuid
       * Format: uuid
       */
      s3_file_uuid: string;
      /** Filename */
      filename: string;
    };
    /** Grade */
    Grade: {
      /** Id */
      id: number;
      /** Receiver Id */
      receiver_id: number;
      /** Grader Id */
      grader_id: number;
      /** Class Id */
      class_id: number;
      /** Title */
      title: string;
      /** Score */
      score: string;
      /** Comment */
      comment: string;
      /**
       * Update Time
       * Format: date-time
       */
      update_time: string;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** JudgeCase */
    JudgeCase: {
      /** Judgment Id */
      judgment_id: number;
      /** Testcase Id */
      testcase_id: number;
      verdict: components['schemas']['VerdictType'];
      /** Time Lapse */
      time_lapse: number;
      /** Peak Memory */
      peak_memory: number;
      /** Score */
      score: number;
    };
    /** JudgeSource */
    JudgeSource: {
      /** Judge Language */
      judge_language: string;
      /**
       * Code Uuid
       * Format: uuid
       */
      code_uuid: string;
      /** Filename */
      filename: string;
    };
    /** JudgeSourceInput */
    JudgeSourceInput: {
      /** Judge Language */
      judge_language: string;
      /** Judge Code */
      judge_code: string;
    };
    /** LoginInput */
    LoginInput: {
      /** Username */
      username: string;
      /** Password */
      password: string;
    };
    /** LoginOutput */
    LoginOutput: {
      /** Token */
      token: string;
      /** Account Id */
      account_id: number;
    };
    /**
     * ProblemJudgeType
     * @description An enumeration.
     * @enum {string}
     */
    ProblemJudgeType: 'NORMAL' | 'CUSTOMIZED';
    /** ProblemReviser */
    ProblemReviser: {
      /** Judge Language */
      judge_language: string;
      /**
       * Code Uuid
       * Format: uuid
       */
      code_uuid: string;
      /** Filename */
      filename: string;
    };
    /** ProblemStatOutput */
    ProblemStatOutput: {
      /** Task Label */
      task_label: string;
      /** Solved Member Count */
      solved_member_count: number;
      /** Submission Count */
      submission_count: number;
      /** Member Count */
      member_count: number;
    };
    /** ReadAccountOutput */
    ReadAccountOutput: {
      /** Id */
      id: number;
      /** Username */
      username: string;
      /** Nickname */
      nickname: string;
      /** Role */
      role: string;
      /** Real Name */
      real_name: string;
      /** Alternative Email */
      alternative_email: string;
      /** Student Id */
      student_id: string;
    };
    /** ReadClassMemberOutput */
    ReadClassMemberOutput: {
      /** Member Id */
      member_id: number;
      /** Member Referral */
      member_referral: string;
      member_role: components['schemas']['RoleType'];
    };
    /** ReadPeerReviewRecordOutput */
    ReadPeerReviewRecordOutput: {
      /** Id */
      id: number;
      /** Peer Review Id */
      peer_review_id: number;
      /** Submission Id */
      submission_id: number;
      /** Grader Id */
      grader_id: number;
      /** Receiver Id */
      receiver_id: number;
      /** Score */
      score: number;
      /** Comment */
      comment: string;
      /**
       * Submit Time
       * Format: date-time
       */
      submit_time: string;
      /** Filename */
      filename: string;
      /**
       * File Uuid
       * Format: uuid
       */
      file_uuid: string;
    };
    /** ReadProblemOutput */
    ReadProblemOutput: {
      /** Id */
      id: number;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Label */
      challenge_label: string;
      judge_type: components['schemas']['ProblemJudgeType'];
      /** Title */
      title: string;
      /** Setter Id */
      setter_id: number;
      /** Full Score */
      full_score: number;
      /** Description */
      description: string;
      /** Io Description */
      io_description: string;
      /** Source */
      source: string;
      /** Hint */
      hint: string;
      /** Is Deleted */
      is_deleted: boolean;
      judge_source: components['schemas']['JudgeSource'];
      /** Reviser Is Enabled */
      reviser_is_enabled: boolean;
      reviser: components['schemas']['ProblemReviser'];
    };
    /** ReadProblemStatusOutput */
    ReadProblemStatusOutput: {
      /** Problem Id */
      problem_id: number;
      /** Submission Id */
      submission_id: number;
    };
    /** ReadScoreboardOutput */
    ReadScoreboardOutput: {
      /** Id */
      id: number;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Target Problem Ids */
      target_problem_ids: number[];
      /** Is Deleted */
      is_deleted: boolean;
      type: components['schemas']['ScoreboardType'];
      /** Data */
      data: unknown;
    };
    /** ReadStatusOutput */
    ReadStatusOutput: {
      /** Problem */
      problem: components['schemas']['ReadProblemStatusOutput'][];
    };
    /** RejudgeProblemOutput */
    RejudgeProblemOutput: {
      /** Submission Count */
      submission_count: number;
    };
    /** ResetPasswordInput */
    ResetPasswordInput: {
      /** Code */
      code: string;
      /** Password */
      password: string;
    };
    /** ReturnEachRun */
    ReturnEachRun: {
      /** Id */
      id: number;
      /** Team */
      team: number;
      /** Problem */
      problem: number;
      /** Result */
      result: string;
      /** Submissiontime */
      submissionTime: number;
    };
    /**
     * ReviserSettingType
     * @description An enumeration.
     * @enum {string}
     */
    ReviserSettingType: 'CUSTOMIZED';
    /**
     * RoleType
     * @description An enumeration.
     * @enum {string}
     */
    RoleType: 'GUEST' | 'NORMAL' | 'MANAGER';
    /** S3FileUrlOutput */
    S3FileUrlOutput: {
      /** Url */
      url: string;
    };
    /** Scoreboard */
    Scoreboard: {
      /** Id */
      id: number;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Target Problem Ids */
      target_problem_ids: number[];
      /** Is Deleted */
      is_deleted: boolean;
      type: components['schemas']['ScoreboardType'];
      /** Setting Id */
      setting_id: number;
    };
    /**
     * ScoreboardType
     * @description An enumeration.
     * @enum {string}
     */
    ScoreboardType: 'TEAM_PROJECT' | 'TEAM_CONTEST';
    /** SetClassMemberInput */
    SetClassMemberInput: {
      /** Account Referral */
      account_referral: string;
      role: components['schemas']['RoleType'];
    };
    /** Submission */
    Submission: {
      /** Id */
      id: number;
      /** Account Id */
      account_id: number;
      /** Problem Id */
      problem_id: number;
      /** Language Id */
      language_id: number;
      /**
       * Content File Uuid
       * Format: uuid
       */
      content_file_uuid: string;
      /** Content Length */
      content_length: number;
      /** Filename */
      filename: string;
      /**
       * Submit Time
       * Format: date-time
       */
      submit_time: string;
    };
    /** SubmissionLanguage */
    SubmissionLanguage: {
      /** Id */
      id: number;
      /** Name */
      name: string;
      /** Version */
      version: string;
      /** Is Disabled */
      is_disabled: boolean;
    };
    /** SubmitPeerReviewInput */
    SubmitPeerReviewInput: {
      /** Score */
      score: number;
      /** Comment */
      comment: string;
    };
    /**
     * TaskSelectionType
     * @description An enumeration.
     * @enum {string}
     */
    TaskSelectionType: 'LAST' | 'BEST';
    /** Team */
    Team: {
      /** Id */
      id: number;
      /** Name */
      name: string;
      /** Class Id */
      class_id: number;
      /** Label */
      label: string;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** TeamMember */
    TeamMember: {
      /** Member Id */
      member_id: number;
      /** Team Id */
      team_id: number;
      role: components['schemas']['RoleType'];
    };
    /** TimeInfo */
    TimeInfo: {
      /** Contesttime */
      contestTime: number;
      /** Nomoreupdate */
      noMoreUpdate: boolean;
      /** Timestamp */
      timestamp: number;
    };
    /**
     * VerdictType
     * @description An enumeration.
     * @enum {string}
     */
    VerdictType:
      | 'ACCEPTED'
      | 'WRONG ANSWER'
      | 'MEMORY LIMIT EXCEED'
      | 'TIME LIMIT EXCEED'
      | 'RUNTIME ERROR'
      | 'COMPILE ERROR'
      | 'CONTACT MANAGER'
      | 'FORBIDDEN ACTION'
      | 'SYSTEM ERROR';
    /** ViewAccessLog */
    ViewAccessLog: {
      /** Account Id */
      account_id: number;
      /** Username */
      username: string;
      /** Student Id */
      student_id: string;
      /** Real Name */
      real_name: string;
      /** Ip */
      ip: string;
      /** Resource Path */
      resource_path: string;
      /** Request Method */
      request_method: string;
      /**
       * Access Time
       * Format: date-time
       */
      access_time: string;
      /** Access Log Id */
      access_log_id: number;
    };
    /** ViewAccessLogOutput */
    ViewAccessLogOutput: {
      /** Data */
      data: components['schemas']['ViewAccessLog'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewAccount */
    ViewAccount: {
      /** Account Id */
      account_id: number;
      /** Username */
      username: string;
      /** Real Name */
      real_name: string;
      /** Student Id */
      student_id: string;
    };
    /** ViewAccountOutput */
    ViewAccountOutput: {
      /** Data */
      data: components['schemas']['ViewAccount'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewClassMember */
    ViewClassMember: {
      /** Account Id */
      account_id: number;
      /** Username */
      username: string;
      /** Student Id */
      student_id: string;
      /** Real Name */
      real_name: string;
      /** Abbreviated Name */
      abbreviated_name: string;
      role: components['schemas']['RoleType'];
      /** Class Id */
      class_id: number;
    };
    /** ViewClassMemberOutput */
    ViewClassMemberOutput: {
      /** Data */
      data: components['schemas']['ViewClassMember'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewGrade */
    ViewGrade: {
      /** Account Id */
      account_id: number;
      /** Username */
      username: string;
      /** Student Id */
      student_id: string;
      /** Real Name */
      real_name: string;
      /** Title */
      title: string;
      /** Score */
      score: string;
      /**
       * Update Time
       * Format: date-time
       */
      update_time: string;
      /** Grade Id */
      grade_id: number;
      /** Class Id */
      class_id: number;
    };
    /** ViewGradeOutput */
    ViewGradeOutput: {
      /** Data */
      data: components['schemas']['ViewGrade'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewMySubmission */
    ViewMySubmission: {
      /** Submission Id */
      submission_id: number;
      /** Course Id */
      course_id: number;
      /** Course Name */
      course_name: string;
      /** Class Id */
      class_id: number;
      /** Class Name */
      class_name: string;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Title */
      challenge_title: string;
      /** Problem Id */
      problem_id: number;
      /** Challenge Label */
      challenge_label: string;
      verdict: components['schemas']['VerdictType'];
      /**
       * Submit Time
       * Format: date-time
       */
      submit_time: string;
      /** Account Id */
      account_id: number;
    };
    /** ViewMySubmissionOutput */
    ViewMySubmissionOutput: {
      /** Data */
      data: components['schemas']['ViewMySubmission'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewMySubmissionUnderProblem */
    ViewMySubmissionUnderProblem: {
      /** Submission Id */
      submission_id: number;
      /** Judgment Id */
      judgment_id: number;
      verdict: components['schemas']['VerdictType'];
      /** Score */
      score: number;
      /** Total Time */
      total_time: number;
      /** Max Memory */
      max_memory: number;
      /**
       * Submit Time
       * Format: date-time
       */
      submit_time: string;
      /** Account Id */
      account_id: number;
      /** Problem Id */
      problem_id: number;
    };
    /** ViewMySubmissionUnderProblemOutput */
    ViewMySubmissionUnderProblemOutput: {
      /** Data */
      data: components['schemas']['ViewMySubmissionUnderProblem'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewProblemSet */
    ViewProblemSet: {
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Title */
      challenge_title: string;
      /** Problem Id */
      problem_id: number;
      /** Challenge Label */
      challenge_label: string;
      /** Problem Title */
      problem_title: string;
      /** Class Id */
      class_id: number;
    };
    /** ViewProblemSetOutput */
    ViewProblemSetOutput: {
      /** Data */
      data: components['schemas']['ViewProblemSet'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewSubmissionUnderClass */
    ViewSubmissionUnderClass: {
      /** Submission Id */
      submission_id: number;
      /** Account Id */
      account_id: number;
      /** Username */
      username: string;
      /** Student Id */
      student_id: string;
      /** Real Name */
      real_name: string;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Title */
      challenge_title: string;
      /** Problem Id */
      problem_id: number;
      /** Challenge Label */
      challenge_label: string;
      verdict: components['schemas']['VerdictType'];
      /**
       * Submit Time
       * Format: date-time
       */
      submit_time: string;
      /** Class Id */
      class_id: number;
    };
    /** ViewSubmissionUnderClassOutput */
    ViewSubmissionUnderClassOutput: {
      /** Data */
      data: components['schemas']['ViewSubmissionUnderClass'][];
      /** Total Count */
      total_count: number;
    };
    /** ViewTeamContestScoreboardOutput */
    ViewTeamContestScoreboardOutput: {
      /** Team Id */
      team_id: number;
      /** Team Name */
      team_name: string;
      /** Total Penalty */
      total_penalty: number;
      /** Solved Problem Count */
      solved_problem_count: number;
      /** Target Problem Data */
      target_problem_data: components['schemas']['ViewTeamContestScoreboardProblemScoreOutput'][];
    };
    /** ViewTeamContestScoreboardProblemScoreOutput */
    ViewTeamContestScoreboardProblemScoreOutput: {
      /** Problem Id */
      problem_id: number;
      /** Submit Count */
      submit_count: number;
      /** Is Solved */
      is_solved: boolean;
      /** Solve Time */
      solve_time: number;
      /** Is First */
      is_first: boolean;
      /** Penalty */
      penalty: number;
      /** Submission Id */
      submission_id: number;
    };
    /** ViewTeamContestScoreboardRunsOutput */
    ViewTeamContestScoreboardRunsOutput: {
      time: components['schemas']['TimeInfo'];
      /** Runs */
      runs: components['schemas']['ReturnEachRun'][];
    };
    /** ViewTeamProjectScoreboardOutput */
    ViewTeamProjectScoreboardOutput: {
      /** Team Id */
      team_id: number;
      /** Team Name */
      team_name: string;
      /** Total Score */
      total_score: number;
      /** Target Problem Data */
      target_problem_data: components['schemas']['ViewTeamProjectScoreboardProblemScoreOutput'][];
    };
    /** ViewTeamProjectScoreboardProblemScoreOutput */
    ViewTeamProjectScoreboardProblemScoreOutput: {
      /** Problem Id */
      problem_id: number;
      /** Score */
      score: number;
      /** Submission Id */
      submission_id: number;
    };
    /** add_account_return */
    add_account_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** add_announcement_return */
    add_announcement_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_assisting_data_under_problem_return */
    add_assisting_data_under_problem_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_challenge_under_class_return */
    add_challenge_under_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_class_under_course_return */
    add_class_under_course_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_course_return */
    add_course_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_essay_under_challenge_return */
    add_essay_under_challenge_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_grade_return */
    add_grade_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_institute_return */
    add_institute_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_normal_account_return */
    add_normal_account_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_peer_review_under_challenge_return */
    add_peer_review_under_challenge_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_problem_under_challenge_return */
    add_problem_under_challenge_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_student_card_to_account_return */
    add_student_card_to_account_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** add_submission_language_return */
    add_submission_language_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_team_member_return */
    add_team_member_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: boolean[];
      /** Error */
      error: string;
    };
    /** add_team_project_scoreboard_under_challenge_return */
    add_team_project_scoreboard_under_challenge_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_team_under_class_return */
    add_team_under_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** add_testcase_under_problem_return */
    add_testcase_under_problem_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** assign_peer_review_record_return */
    assign_peer_review_record_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['AssignPeerReviewOutput'];
      /** Error */
      error: string;
    };
    /** batch_get_account_by_account_referrals_return */
    batch_get_account_by_account_referrals_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__BatchGetAccountOutput'][];
      /** Error */
      error: string;
    };
    /** batch_get_account_with_default_student_id_return */
    batch_get_account_with_default_student_id_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__BatchGetAccountOutput'][];
      /** Error */
      error: string;
    };
    /** batch_get_submission_judgment_return */
    batch_get_submission_judgment_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__Judgment'][];
      /** Error */
      error: string;
    };
    /** browse_account_grade_return */
    browse_account_grade_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_account_received_peer_review_record_return */
    browse_account_received_peer_review_record_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: number[];
      /** Error */
      error: string;
    };
    /** browse_account_reviewed_peer_review_record_return */
    browse_account_reviewed_peer_review_record_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: number[];
      /** Error */
      error: string;
    };
    /** browse_all_account_pending_email_verification_return */
    browse_all_account_pending_email_verification_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['EmailVerification'][];
      /** Error */
      error: string;
    };
    /** browse_all_account_student_card_return */
    browse_all_account_student_card_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__StudentCard'][];
      /** Error */
      error: string;
    };
    /** browse_all_account_with_class_role_return */
    browse_all_account_with_class_role_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['BrowseAccountWithRoleOutput'][];
      /** Error */
      error: string;
    };
    /** browse_all_assisting_data_under_problem_return */
    browse_all_assisting_data_under_problem_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__ReadAssistingDataOutput'][];
      /** Error */
      error: string;
    };
    /** browse_all_class_member_with_account_referral_return */
    browse_all_class_member_with_account_referral_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['ReadClassMemberOutput'][];
      /** Error */
      error: string;
    };
    /** browse_all_class_under_course_return */
    browse_all_class_under_course_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__Class'][];
      /** Error */
      error: string;
    };
    /** browse_all_course_return */
    browse_all_course_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__Course'][];
      /** Error */
      error: string;
    };
    /** browse_all_institute_return */
    browse_all_institute_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__Institute'][];
      /** Error */
      error: string;
    };
    /** browse_all_judgment_judge_case_return */
    browse_all_judgment_judge_case_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['JudgeCase'][];
      /** Error */
      error: string;
    };
    /** browse_all_judgment_verdict_return */
    browse_all_judgment_verdict_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['VerdictType'][];
      /** Error */
      error: string;
    };
    /** browse_all_submission_judgment_return */
    browse_all_submission_judgment_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__Judgment'][];
      /** Error */
      error: string;
    };
    /** browse_all_submission_language_return */
    browse_all_submission_language_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['SubmissionLanguage'][];
      /** Error */
      error: string;
    };
    /** browse_all_task_status_under_challenge_return */
    browse_all_task_status_under_challenge_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['ReadStatusOutput'][];
      /** Error */
      error: string;
    };
    /** browse_all_task_under_challenge_return */
    browse_all_task_under_challenge_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['BrowseTaskOutput'];
      /** Error */
      error: string;
    };
    /** browse_all_testcase_under_problem_return */
    browse_all_testcase_under_problem_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__ReadTestcaseOutput'][];
      /** Error */
      error: string;
    };
    /** browse_announcement_return */
    browse_announcement_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_challenge_under_class_return */
    browse_challenge_under_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_class_return */
    browse_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_essay_submission_by_essay_id_return */
    browse_essay_submission_by_essay_id_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_my_submission_under_problem_return */
    browse_my_submission_under_problem_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ViewMySubmissionUnderProblemOutput'];
      /** Error */
      error: string;
    };
    /** browse_peer_review_record_return */
    browse_peer_review_record_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_problem_set_return */
    browse_problem_set_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['pydantic__dataclasses__Problem'][];
      /** Error */
      error: string;
    };
    /** browse_problem_set_under_class_return */
    browse_problem_set_under_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ViewProblemSetOutput'];
      /** Error */
      error: string;
    };
    /** browse_team_all_member_return */
    browse_team_all_member_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['TeamMember'][];
      /** Error */
      error: string;
    };
    /** browse_team_under_class_return */
    browse_team_under_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** delete_account_return */
    delete_account_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_announcement_return */
    delete_announcement_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_assisting_data_return */
    delete_assisting_data_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_challenge_return */
    delete_challenge_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_class_member_return */
    delete_class_member_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_class_return */
    delete_class_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_course_return */
    delete_course_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_essay_return */
    delete_essay_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_grade_return */
    delete_grade_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_peer_review_return */
    delete_peer_review_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_pending_email_verification_return */
    delete_pending_email_verification_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_problem_return */
    delete_problem_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_scoreboard_return */
    delete_scoreboard_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_team_member_return */
    delete_team_member_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_team_return */
    delete_team_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_testcase_input_data_return */
    delete_testcase_input_data_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_testcase_output_data_return */
    delete_testcase_output_data_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** delete_testcase_return */
    delete_testcase_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** download_all_assisting_data_return */
    download_all_assisting_data_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** download_all_essay_submission_return */
    download_all_essay_submission_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** download_all_non_sample_testcase_return */
    download_all_non_sample_testcase_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** download_all_plagiarism_reports_return */
    download_all_plagiarism_reports_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** download_all_sample_testcase_return */
    download_all_sample_testcase_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** download_all_submissions_return */
    download_all_submissions_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_account_return */
    edit_account_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_announcement_return */
    edit_announcement_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_assisting_data_return */
    edit_assisting_data_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_challenge_return */
    edit_challenge_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_class_return */
    edit_class_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_course_return */
    edit_course_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_essay_return */
    edit_essay_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_grade_return */
    edit_grade_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_institute_return */
    edit_institute_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_password_return */
    edit_password_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_peer_review_return */
    edit_peer_review_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_problem_return */
    edit_problem_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_submission_language_return */
    edit_submission_language_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_team_contest_scoreboard_return */
    edit_team_contest_scoreboard_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_team_member_return */
    edit_team_member_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_team_project_scoreboard_return */
    edit_team_project_scoreboard_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_team_return */
    edit_team_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** edit_testcase_return */
    edit_testcase_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** forget_password_return */
    forget_password_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** forget_username_return */
    forget_username_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** get_account_template_file_return */
    get_account_template_file_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['GetAccountTemplateOutput'];
      /** Error */
      error: string;
    };
    /** get_challenge_statistics_return */
    get_challenge_statistics_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['GetChallengeStatOutput'];
      /** Error */
      error: string;
    };
    /** get_grade_return */
    get_grade_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['Grade'];
      /** Error */
      error: string;
    };
    /** get_grade_template_file_return */
    get_grade_template_file_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['GetGradeTemplateOutput'];
      /** Error */
      error: string;
    };
    /** get_member_submission_statistics_return */
    get_member_submission_statistics_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** get_problem_statistics_return */
    get_problem_statistics_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['GetProblemStatOutput'];
      /** Error */
      error: string;
    };
    /** get_s3_file_url_return */
    get_s3_file_url_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['S3FileUrlOutput'];
      /** Error */
      error: string;
    };
    /** get_score_by_best_under_problem_return */
    get_score_by_best_under_problem_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__GetScoreByTypeOutput'];
      /** Error */
      error: string;
    };
    /** get_score_by_challenge_type_under_problem_return */
    get_score_by_challenge_type_under_problem_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__GetScoreByTypeOutput'];
      /** Error */
      error: string;
    };
    /** get_team_template_file_return */
    get_team_template_file_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['GetTeamTemplateOutput'];
      /** Error */
      error: string;
    };
    /** import_account_return */
    import_account_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** import_class_grade_return */
    import_class_grade_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** import_team_return */
    import_team_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** login_return */
    login_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['LoginOutput'];
      /** Error */
      error: string;
    };
    /** make_student_card_default_return */
    make_student_card_default_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** peer_review_summary_receive_return */
    peer_review_summary_receive_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__ViewPeerReviewRecordOutput'];
      /** Error */
      error: string;
    };
    /** peer_review_summary_review_return */
    peer_review_summary_review_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__ViewPeerReviewRecordOutput'];
      /** Error */
      error: string;
    };
    /** EditScoreboardInput */
    processor__http__scoreboard_setting_team_contest__EditScoreboardInput: {
      /** Challenge Label */
      challenge_label?: string;
      /** Title */
      title?: string;
      /** Target Problem Ids */
      target_problem_ids?: number[];
      /** Penalty Formula */
      penalty_formula?: string;
      /** Team Label Filter */
      team_label_filter?: string;
    };
    /** EditScoreboardInput */
    processor__http__scoreboard_setting_team_project__EditScoreboardInput: {
      /** Challenge Label */
      challenge_label?: string;
      /** Title */
      title?: string;
      /** Target Problem Ids */
      target_problem_ids?: number[];
      /** Scoring Formula */
      scoring_formula?: string;
      /** Baseline Team Id */
      baseline_team_id?: number;
      /** Rank By Total Score */
      rank_by_total_score?: boolean;
      /** Team Label Filter */
      team_label_filter?: string;
    };
    /** AddOutput */
    pydantic__dataclasses__AddOutput: {
      /** Id */
      id: number;
    };
    /** BatchGetAccountOutput */
    pydantic__dataclasses__BatchGetAccountOutput: {
      /** Id */
      id: number;
      /** Username */
      username: string;
      /** Real Name */
      real_name: string;
      /** Student Id */
      student_id: string;
    };
    /** BrowseOutputBase */
    pydantic__dataclasses__BrowseOutputBase: {
      /** Data */
      data: unknown;
      /** Total Count */
      total_count: number;
    };
    /** Class */
    pydantic__dataclasses__Class: {
      /** Id */
      id: number;
      /** Name */
      name: string;
      /** Course Id */
      course_id: number;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** Course */
    pydantic__dataclasses__Course: {
      /** Id */
      id: number;
      /** Name */
      name: string;
      type: components['schemas']['CourseType'];
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** Essay */
    pydantic__dataclasses__Essay: {
      /** Id */
      id: number;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Setter Id */
      setter_id: number;
      /** Description */
      description: string;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** GetScoreByTypeOutput */
    pydantic__dataclasses__GetScoreByTypeOutput: {
      challenge_type: components['schemas']['TaskSelectionType'];
      /** Score */
      score: number;
    };
    /** Institute */
    pydantic__dataclasses__Institute: {
      /** Id */
      id: number;
      /** Abbreviated Name */
      abbreviated_name: string;
      /** Full Name */
      full_name: string;
      /** Email Domain */
      email_domain: string;
      /** Is Disabled */
      is_disabled: boolean;
    };
    /** Judgment */
    pydantic__dataclasses__Judgment: {
      /** Id */
      id: number;
      /** Submission Id */
      submission_id: number;
      verdict: components['schemas']['VerdictType'];
      /** Total Time */
      total_time: number;
      /** Max Memory */
      max_memory: number;
      /** Score */
      score: number;
      /** Error Message */
      error_message: string;
      /**
       * Judge Time
       * Format: date-time
       */
      judge_time: string;
    };
    /** PeerReview */
    pydantic__dataclasses__PeerReview: {
      /** Id */
      id: number;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Label */
      challenge_label: string;
      /** Title */
      title: string;
      /** Target Problem Id */
      target_problem_id: number;
      /** Setter Id */
      setter_id: number;
      /** Description */
      description: string;
      /** Min Score */
      min_score: number;
      /** Max Score */
      max_score: number;
      /** Max Review Count */
      max_review_count: number;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** Problem */
    pydantic__dataclasses__Problem: {
      /** Id */
      id: number;
      /** Challenge Id */
      challenge_id: number;
      /** Challenge Label */
      challenge_label: string;
      judge_type: components['schemas']['ProblemJudgeType'];
      /** Setting Id */
      setting_id: number;
      /** Title */
      title: string;
      /** Setter Id */
      setter_id: number;
      /** Full Score */
      full_score: number;
      /** Description */
      description: string;
      /** Io Description */
      io_description: string;
      /** Source */
      source: string;
      /** Hint */
      hint: string;
      /** Is Lazy Judge */
      is_lazy_judge: boolean;
      /** Is Deleted */
      is_deleted: boolean;
      /** Reviser Settings */
      reviser_settings: components['schemas']['pydantic__dataclasses__ProblemReviserSetting'][];
    };
    /** ProblemReviserSetting */
    pydantic__dataclasses__ProblemReviserSetting: {
      /** Id */
      id: number;
      type: components['schemas']['ReviserSettingType'];
    };
    /** ReadAssistingDataOutput */
    pydantic__dataclasses__ReadAssistingDataOutput: {
      /** Id */
      id: number;
      /** Problem Id */
      problem_id: number;
      /**
       * S3 File Uuid
       * Format: uuid
       */
      s3_file_uuid: string;
      /** Filename */
      filename: string;
    };
    /** ReadTestcaseOutput */
    pydantic__dataclasses__ReadTestcaseOutput: {
      /** Id */
      id: number;
      /** Problem Id */
      problem_id: number;
      /** Is Sample */
      is_sample: boolean;
      /** Score */
      score: number;
      /** Label */
      label: string;
      /**
       * Input File Uuid
       * Format: uuid
       */
      input_file_uuid: string;
      /**
       * Output File Uuid
       * Format: uuid
       */
      output_file_uuid: string;
      /** Input Filename */
      input_filename: string;
      /** Output Filename */
      output_filename: string;
      /** Note */
      note: string;
      /** Time Limit */
      time_limit: number;
      /** Memory Limit */
      memory_limit: number;
      /** Is Disabled */
      is_disabled: boolean;
      /** Is Deleted */
      is_deleted: boolean;
    };
    /** StudentCard */
    pydantic__dataclasses__StudentCard: {
      /** Id */
      id: number;
      /** Institute Id */
      institute_id: number;
      /** Student Id */
      student_id: string;
      /** Email */
      email: string;
      /** Is Default */
      is_default: boolean;
    };
    /** ViewPeerReviewRecord */
    pydantic__dataclasses__ViewPeerReviewRecord: {
      /** Account Id */
      account_id: number;
      /** Username */
      username: string;
      /** Real Name */
      real_name: string;
      /** Student Id */
      student_id: string;
      /** Peer Review Record Ids */
      peer_review_record_ids: number[];
      /** Peer Review Record Scores */
      peer_review_record_scores: number[];
      /** Average Score */
      average_score: number;
    };
    /** ViewPeerReviewRecordOutput */
    pydantic__dataclasses__ViewPeerReviewRecordOutput: {
      /** Data */
      data: components['schemas']['pydantic__dataclasses__ViewPeerReviewRecord'][];
      /** Total Count */
      total_count: number;
    };
    /** browse_access_log_return */
    pydantic__dataclasses__browse_access_log_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ViewAccessLogOutput'];
      /** Error */
      error: string;
    };
    /** browse_account_with_default_student_id_return */
    pydantic__dataclasses__browse_account_with_default_student_id_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_class_grade_return */
    pydantic__dataclasses__browse_class_grade_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_class_member_return */
    pydantic__dataclasses__browse_class_member_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_submission_return */
    pydantic__dataclasses__browse_submission_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** browse_submission_under_class_return */
    pydantic__dataclasses__browse_submission_under_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__BrowseOutputBase'];
      /** Error */
      error: string;
    };
    /** email_verification_return */
    pydantic__dataclasses__email_verification_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** read_account_with_default_student_id_return */
    read_account_with_default_student_id_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ReadAccountOutput'];
      /** Error */
      error: string;
    };
    /** read_announcement_return */
    read_announcement_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['Announcement'];
      /** Error */
      error: string;
    };
    /** read_assisting_data_return */
    read_assisting_data_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__ReadAssistingDataOutput'];
      /** Error */
      error: string;
    };
    /** read_challenge_return */
    read_challenge_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['Challenge'];
      /** Error */
      error: string;
    };
    /** read_class_return */
    read_class_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__Class'];
      /** Error */
      error: string;
    };
    /** read_course_return */
    read_course_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__Course'];
      /** Error */
      error: string;
    };
    /** read_essay_return */
    read_essay_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__Essay'];
      /** Error */
      error: string;
    };
    /** read_essay_submission_return */
    read_essay_submission_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['EssaySubmission'];
      /** Error */
      error: string;
    };
    /** read_institute_return */
    read_institute_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__Institute'];
      /** Error */
      error: string;
    };
    /** read_judgment_return */
    read_judgment_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__Judgment'];
      /** Error */
      error: string;
    };
    /** read_peer_review_record_return */
    read_peer_review_record_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ReadPeerReviewRecordOutput'];
      /** Error */
      error: string;
    };
    /** read_peer_review_return */
    read_peer_review_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__PeerReview'];
      /** Error */
      error: string;
    };
    /** read_problem_return */
    read_problem_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ReadProblemOutput'];
      /** Error */
      error: string;
    };
    /** read_scoreboard_return */
    read_scoreboard_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ReadScoreboardOutput'];
      /** Error */
      error: string;
    };
    /** read_student_card_return */
    read_student_card_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__StudentCard'];
      /** Error */
      error: string;
    };
    /** read_submission_latest_judgment_return */
    read_submission_latest_judgment_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__Judgment'];
      /** Error */
      error: string;
    };
    /** read_submission_return */
    read_submission_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['Submission'];
      /** Error */
      error: string;
    };
    /** read_team_return */
    read_team_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['Team'];
      /** Error */
      error: string;
    };
    /** read_testcase_return */
    read_testcase_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__ReadTestcaseOutput'];
      /** Error */
      error: string;
    };
    /** rejudge_problem_return */
    rejudge_problem_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['RejudgeProblemOutput'];
      /** Error */
      error: string;
    };
    /** rejudge_submission_return */
    rejudge_submission_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** replace_class_members_return */
    replace_class_members_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: boolean[];
      /** Error */
      error: string;
    };
    /** resend_email_verification_return */
    resend_email_verification_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** reset_password_return */
    reset_password_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** reupload_essay_return */
    reupload_essay_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** submit_peer_review_record_return */
    submit_peer_review_record_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** submit_return */
    submit_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['pydantic__dataclasses__AddOutput'];
      /** Error */
      error: string;
    };
    /** upload_essay_return */
    upload_essay_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: number;
      /** Error */
      error: string;
    };
    /** upload_testcase_input_data_return */
    upload_testcase_input_data_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** upload_testcase_output_data_return */
    upload_testcase_output_data_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: null;
      /** Error */
      error: string;
    };
    /** view_team_contest_scoreboard_return */
    view_team_contest_scoreboard_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['ViewTeamContestScoreboardOutput'][];
      /** Error */
      error: string;
    };
    /** view_team_contest_scoreboard_runs_return */
    view_team_contest_scoreboard_runs_return: {
      /** Success */
      success: boolean;
      data: components['schemas']['ViewTeamContestScoreboardRunsOutput'];
      /** Error */
      error: string;
    };
    /** view_team_project_scoreboard_return */
    view_team_project_scoreboard_return: {
      /** Success */
      success: boolean;
      /** Data */
      data: components['schemas']['ViewTeamProjectScoreboardOutput'][];
      /** Error */
      error: string;
    };
  };
  responses: never;
  parameters: never;
  requestBodies: never;
  headers: never;
  pathItems: never;
}

export type external = Record<string, never>;

export interface operations {
  /** Default Page */
  default_page__get: {
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'text/html': string;
        };
      };
    };
  };
  /** Email Verification */
  email_verification_email_verification_get: {
    parameters: {
      query: {
        code: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__email_verification_return'];
        };
      };
    };
  };
  /** Email Verification */
  email_verification_email_verification_post: {
    parameters: {
      query: {
        code: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__email_verification_return'];
        };
      };
    };
  };
  /** Forget Password */
  forget_password_account_forget_password_post: {
    requestBody: {
      content: {
        'application/json': components['schemas']['ForgetPasswordInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['forget_password_return'];
        };
      };
    };
  };
  /** Forget Username */
  forget_username_account_forget_username_post: {
    requestBody: {
      content: {
        'application/json': components['schemas']['ForgetUsernameInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['forget_username_return'];
        };
      };
    };
  };
  /**
   * Get S3 File Url
   * @description ### 權限
   * - SN
   *
   * ### Note
   * - 目前所有 url 都有時間限制 (超時會自動過期)
   */
  get_s3_file_url_s3_file__s3_file_uuid__url_get: {
    parameters: {
      query: {
        filename: string;
        as_attachment: boolean;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        s3_file_uuid: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_s3_file_url_return'];
        };
      };
    };
  };
  /**
   * Browse All Institute
   * @description ### 權限
   * - Public
   */
  browse_all_institute_institute_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_institute_return'];
        };
      };
    };
  };
  /**
   * Add Institute
   * @description ### 權限
   * - System Manager
   */
  add_institute_institute_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddInstituteInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_institute_return'];
        };
      };
    };
  };
  /**
   * Read Institute
   * @description ### 權限
   * - Public
   */
  read_institute_institute__institute_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        institute_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_institute_return'];
        };
      };
    };
  };
  /**
   * Edit Institute
   * @description ### 權限
   * - System Manager
   */
  edit_institute_institute__institute_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        institute_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditInstituteInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_institute_return'];
        };
      };
    };
  };
  /**
   * Browse Account With Default Student Id
   * @description ### 權限
   * - System Manager
   * {'id': 'int', 'username': 'str', 'nickname': 'str', 'role': 'str', 'real_name': 'str', 'alternative_email': 'str'}
   */
  browse_account_with_default_student_id_account_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_account_with_default_student_id_return'];
        };
      };
    };
  };
  /** Add Account */
  add_account_account_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddAccountInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_account_return'];
        };
      };
    };
  };
  /**
   * Batch Get Account With Default Student Id
   * @description ### 權限
   * - System Normal
   *
   * ### Notes
   * - `account_ids`: list of int
   */
  batch_get_account_with_default_student_id_account_summary_batch_get: {
    parameters: {
      query: {
        account_ids: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['batch_get_account_with_default_student_id_return'];
        };
      };
    };
  };
  /**
   * Batch Get Account By Account Referrals
   * @description ### 權限
   * - System Normal
   *
   * ### Notes:
   * account_referrals: list of string
   */
  batch_get_account_by_account_referrals_account_summary_batch_by_account_referral_get: {
    parameters: {
      query: {
        account_referrals: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['batch_get_account_by_account_referrals_return'];
        };
      };
    };
  };
  /**
   * Browse All Account With Class Role
   * @description ### 權限
   * - Self
   *
   * ### Available columns
   */
  browse_all_account_with_class_role_account__account_id__class_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_account_with_class_role_return'];
        };
      };
    };
  };
  /**
   * Get Account Template File
   * @description ### 權限
   * - System Manager
   */
  get_account_template_file_account_template_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_account_template_file_return'];
        };
      };
    };
  };
  /**
   * Read Account With Default Student Id
   * @description ### 權限
   * - System Manager
   * - Self
   * - System Normal (個資除外)
   */
  read_account_with_default_student_id_account__account_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_account_with_default_student_id_return'];
        };
      };
    };
  };
  /**
   * Delete Account
   * @description ### 權限
   * - System manager
   * - Self
   */
  delete_account_account__account_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_account_return'];
        };
      };
    };
  };
  /**
   * Edit Account
   * @description ### 權限
   * - System Manager
   * - Self
   */
  edit_account_account__account_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditAccountInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_account_return'];
        };
      };
    };
  };
  /**
   * Make Student Card Default
   * @description ### 權限
   * - System manager
   * - Self
   */
  make_student_card_default_account__account_id__default_student_card_put: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['DefaultStudentCardInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['make_student_card_default_return'];
        };
      };
    };
  };
  /**
   * Browse All Account Pending Email Verification
   * @description ### 權限
   * - System manager
   * - Self
   */
  browse_all_account_pending_email_verification_account__account_id__email_verification_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_account_pending_email_verification_return'];
        };
      };
    };
  };
  /**
   * Browse All Account Student Card
   * @description ### 權限
   * - System manager
   * - Self
   */
  browse_all_account_student_card_account__account_id__student_card_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_account_student_card_return'];
        };
      };
    };
  };
  /**
   * Add Student Card To Account
   * @description ### 權限
   * - System manager
   * - Self
   */
  add_student_card_to_account_account__account_id__student_card_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddStudentCardInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_student_card_to_account_return'];
        };
      };
    };
  };
  /**
   * Read Student Card
   * @description ### 權限
   * - System manager
   * - Self
   */
  read_student_card_student_card__student_card_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        student_card_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_student_card_return'];
        };
      };
    };
  };
  /**
   * Resend Email Verification
   * @description ### 權限
   * - System manager
   * - Self
   */
  resend_email_verification_email_verification__email_verification_id__resend_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        email_verification_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['resend_email_verification_return'];
        };
      };
    };
  };
  /**
   * Delete Pending Email Verification
   * @description ### 權限
   * - System manager
   * - Self
   */
  delete_pending_email_verification_email_verification__email_verification_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        email_verification_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_pending_email_verification_return'];
        };
      };
    };
  };
  /**
   * Browse All Course
   * @description ### 權限
   * - System manager (hidden)
   * - System normal (not hidden)
   */
  browse_all_course_course_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_course_return'];
        };
      };
    };
  };
  /**
   * Add Course
   * @description ### 權限
   * - System manager
   */
  add_course_course_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddCourseInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_course_return'];
        };
      };
    };
  };
  /**
   * Read Course
   * @description ### 權限
   * - System manager (hidden)
   * - System normal (not hidden)
   */
  read_course_course__course_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        course_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_course_return'];
        };
      };
    };
  };
  /**
   * Delete Course
   * @description ### 權限
   * - System manager
   */
  delete_course_course__course_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        course_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_course_return'];
        };
      };
    };
  };
  /**
   * Edit Course
   * @description ### 權限
   * - System manager
   */
  edit_course_course__course_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        course_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditCourseInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_course_return'];
        };
      };
    };
  };
  /**
   * Browse All Class Under Course
   * @description ### 權限
   * - Class+ manager (hidden)
   * - System normal (not hidden)
   */
  browse_all_class_under_course_course__course_id__class_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        course_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_class_under_course_return'];
        };
      };
    };
  };
  /**
   * Add Class Under Course
   * @description ### 權限
   * - System manager
   */
  add_class_under_course_course__course_id__class_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        course_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddClassInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_class_under_course_return'];
        };
      };
    };
  };
  /**
   * Browse Class
   * @description ### 權限
   * - system normal: all
   *
   * ### Available columns
   * {'id': 'int', 'name': 'str', 'course_id': 'int', 'is_deleted': 'bool'}
   */
  browse_class_class_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_class_return'];
        };
      };
    };
  };
  /**
   * Read Class
   * @description ### 權限
   * - System normal: all
   */
  read_class_class__class_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_class_return'];
        };
      };
    };
  };
  /**
   * Delete Class
   * @description ### 權限
   * - System manager
   */
  delete_class_class__class_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_class_return'];
        };
      };
    };
  };
  /**
   * Edit Class
   * @description ### 權限
   * - Class+ manager
   */
  edit_class_class__class_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditClassInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_class_return'];
        };
      };
    };
  };
  /**
   * Browse Class Member
   * @description ### 權限
   * - Class normal
   * - Class+ manager
   *
   * ### Available columns
   * {'member_id': 'int', 'class_id': 'int', 'role': 'RoleType'}
   */
  browse_class_member_class__class_id__member_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_class_member_return'];
        };
      };
    };
  };
  /**
   * Replace Class Members
   * @description ### 權限
   * - Class+ manager
   */
  replace_class_members_class__class_id__member_put: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['SetClassMemberInput'][];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['replace_class_members_return'];
        };
      };
    };
  };
  /**
   * Browse All Class Member With Account Referral
   * @description ### 權限
   * - Class normal
   * - Class+ manager
   */
  browse_all_class_member_with_account_referral_class__class_id__member_account_referral_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_class_member_with_account_referral_return'];
        };
      };
    };
  };
  /**
   * Delete Class Member
   * @description ### 權限
   * - Class+ manager
   */
  delete_class_member_class__class_id__member__member_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
        member_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_class_member_return'];
        };
      };
    };
  };
  /**
   * Browse Team Under Class
   * @description ### 權限
   * - Class normal: all
   *
   * ### Available columns
   * {'id': 'int', 'name': 'str', 'class_id': 'int', 'label': 'str', 'is_deleted': 'bool'}
   */
  browse_team_under_class_class__class_id__team_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_team_under_class_return'];
        };
      };
    };
  };
  /**
   * Add Team Under Class
   * @description ### 權限
   * - Class manager
   */
  add_team_under_class_class__class_id__team_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddTeamInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_team_under_class_return'];
        };
      };
    };
  };
  /**
   * Browse Submission Under Class
   * @description ### 權限
   * - Class manager
   *
   * ### Available columns
   * {'id': 'int', 'account_id': 'int', 'problem_id': 'int', 'language_id': 'int', 'content_file_uuid': 'UUID', 'content_length': 'int', 'filename': 'str', 'submit_time': 'ServerTZDatetime'}
   */
  browse_submission_under_class_class__class_id__submission_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_submission_under_class_return'];
        };
      };
    };
  };
  /**
   * Import Team
   * @description ### 權限
   * - Class manager
   */
  import_team_class__class_id__team_import_post: {
    parameters: {
      query: {
        label: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_import_team_class__class_id__team_import_post'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['import_team_return'];
        };
      };
    };
  };
  /**
   * Get Team Template File
   * @description ### 權限
   * - system normal
   */
  get_team_template_file_team_template_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_team_template_file_return'];
        };
      };
    };
  };
  /**
   * Read Team
   * @description ### 權限
   * - Class normal
   */
  read_team_team__team_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        team_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_team_return'];
        };
      };
    };
  };
  /**
   * Delete Team
   * @description ### 權限
   * - Class manager
   */
  delete_team_team__team_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        team_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_team_return'];
        };
      };
    };
  };
  /**
   * Edit Team
   * @description ### 權限
   * - Class manager
   * - Team manager (limited)
   */
  edit_team_team__team_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        team_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditTeamInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_team_return'];
        };
      };
    };
  };
  /**
   * Browse Team All Member
   * @description ### 權限
   * - Class normal
   */
  browse_team_all_member_team__team_id__member_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        team_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_team_all_member_return'];
        };
      };
    };
  };
  /**
   * Add Team Member
   * @description ### 權限
   * - class manager
   */
  add_team_member_team__team_id__member_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        team_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddMemberInput'][];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_team_member_return'];
        };
      };
    };
  };
  /**
   * Edit Team Member
   * @description ### 權限
   * - Class manager
   */
  edit_team_member_team__team_id__member_patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        team_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditMemberInput'][];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_team_member_return'];
        };
      };
    };
  };
  /**
   * Delete Team Member
   * @description ### 權限
   * - Class manager
   */
  delete_team_member_team__team_id__member__member_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        team_id: number;
        member_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_team_member_return'];
        };
      };
    };
  };
  /**
   * Import Class Grade
   * @description ### 權限
   * - Class manager
   */
  import_class_grade_class__class_id__grade_import_post: {
    parameters: {
      query: {
        title: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_import_class_grade_class__class_id__grade_import_post'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['import_class_grade_return'];
        };
      };
    };
  };
  /**
   * Browse Class Grade
   * @description ### 權限
   * - Class manager (all)
   * - Class normal (self)
   *
   * ### Available columns
   * {'receiver_id': 'int', 'grader_id': 'int', 'class_id': 'int', 'title': 'str', 'score': 'str', 'comment': 'str', 'update_time': 'str'}
   */
  browse_class_grade_class__class_id__grade_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_class_grade_return'];
        };
      };
    };
  };
  /**
   * Add Grade
   * @description ### 權限
   * - Class Manager
   */
  add_grade_class__class_id__grade_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddGradeInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_grade_return'];
        };
      };
    };
  };
  /**
   * Browse Account Grade
   * @description ### 權限
   * - Self
   *
   * ### Available columns
   * {'class_id': 'int', 'title': 'str', 'score': 'str', 'comment': 'str', 'update_time': 'str'}
   */
  browse_account_grade_account__account_id__grade_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_account_grade_return'];
        };
      };
    };
  };
  /**
   * Get Grade Template File
   * @description ### 權限
   * - system normal
   */
  get_grade_template_file_grade_template_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_grade_template_file_return'];
        };
      };
    };
  };
  /**
   * Get Grade
   * @description ### 權限
   * - Class manager (all)
   * - Class normal (self)
   */
  get_grade_grade__grade_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        grade_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_grade_return'];
        };
      };
    };
  };
  /**
   * Delete Grade
   * @description ### 權限
   * - Class manager
   */
  delete_grade_grade__grade_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        grade_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_grade_return'];
        };
      };
    };
  };
  /**
   * Edit Grade
   * @description ### 權限
   * - Class manager
   */
  edit_grade_grade__grade_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        grade_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditGradeInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_grade_return'];
        };
      };
    };
  };
  /**
   * Browse Challenge Under Class
   * @description ### 權限
   * - Class manager (all)
   * - System normal (not scheduled)
   *
   * ### Available columns
   * {'id': 'int', 'class_id': 'int', 'publicize_type': 'ChallengePublicizeType', 'selection_type': 'TaskSelectionType', 'title': 'str', 'setter_id': 'int', 'description': 'str', 'start_time': 'ServerTZDatetime', 'end_time': 'ServerTZDatetime', 'is_deleted': 'bool'}
   */
  browse_challenge_under_class_class__class_id__challenge_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_challenge_under_class_return'];
        };
      };
    };
  };
  /**
   * Add Challenge Under Class
   * @description ### 權限
   * - Class manager
   */
  add_challenge_under_class_class__class_id__challenge_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddChallengeInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_challenge_under_class_return'];
        };
      };
    };
  };
  /**
   * Read Challenge
   * @description ### 權限
   * - Class manager (all)
   * - Class normal & guest (after start time)
   * - System normal (after scheduled time)
   */
  read_challenge_challenge__challenge_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_challenge_return'];
        };
      };
    };
  };
  /**
   * Delete Challenge
   * @description ### 權限
   * - Class manager
   */
  delete_challenge_challenge__challenge_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_challenge_return'];
        };
      };
    };
  };
  /**
   * Edit Challenge
   * @description ### 權限
   * - Class manager
   */
  edit_challenge_challenge__challenge_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditChallengeInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_challenge_return'];
        };
      };
    };
  };
  /**
   * Add Problem Under Challenge
   * @description ### 權限
   * - Class manager
   */
  add_problem_under_challenge_challenge__challenge_id__problem_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddProblemInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_problem_under_challenge_return'];
        };
      };
    };
  };
  /**
   * Add Essay Under Challenge
   * @description ### 權限
   * - Class manager
   */
  add_essay_under_challenge_challenge__challenge_id__essay_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddEssayInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_essay_under_challenge_return'];
        };
      };
    };
  };
  /**
   * Add Peer Review Under Challenge
   * @description ### 權限
   * - Class manager
   */
  add_peer_review_under_challenge_challenge__challenge_id__peer_review_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddPeerReviewInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_peer_review_under_challenge_return'];
        };
      };
    };
  };
  /**
   * Add Team Project Scoreboard Under Challenge
   * @description ### 權限
   * - Class manager
   */
  add_team_project_scoreboard_under_challenge_challenge__challenge_id__team_project_scoreboard_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddTeamProjectScoreboardInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_team_project_scoreboard_under_challenge_return'];
        };
      };
    };
  };
  /**
   * Browse All Task Under Challenge
   * @description ### 權限
   * - Class manager (all)
   * - Class guest (active/archived challenges)
   * - System Normal (by challenge publicize type)
   */
  browse_all_task_under_challenge_challenge__challenge_id__task_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_task_under_challenge_return'];
        };
      };
    };
  };
  /**
   * Browse All Task Status Under Challenge
   * @description ### 權限
   * - Self: see self
   */
  browse_all_task_status_under_challenge_challenge__challenge_id__task_status_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_task_status_under_challenge_return'];
        };
      };
    };
  };
  /**
   * Get Challenge Statistics
   * @description ### 權限
   * - class manager
   */
  get_challenge_statistics_challenge__challenge_id__statistics_summary_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_challenge_statistics_return'];
        };
      };
    };
  };
  /**
   * Get Member Submission Statistics
   * @description ### 權限
   * - class manager
   */
  get_member_submission_statistics_challenge__challenge_id__statistics_member_submission_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_member_submission_statistics_return'];
        };
      };
    };
  };
  /**
   * Download All Submissions
   * @description ### 權限
   * - class manager
   */
  download_all_submissions_challenge__challenge_id__all_submission_post: {
    parameters: {
      query: {
        as_attachment: boolean;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['download_all_submissions_return'];
        };
      };
    };
  };
  /**
   * Download All Plagiarism Reports
   * @description ### 權限
   * - class manager
   */
  download_all_plagiarism_reports_challenge__challenge_id__all_plagiarism_report_post: {
    parameters: {
      query: {
        as_attachment: boolean;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['download_all_plagiarism_reports_return'];
        };
      };
    };
  };
  /**
   * Add Team Contest Scoreboard Under Challenge
   * @description ### 權限
   * - class manager
   */
  add_team_contest_scoreboard_under_challenge_challenge__challenge_id__team_contest_scoreboard_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        challenge_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddTeamContestScoreboardInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__AddOutput'];
        };
      };
    };
  };
  /**
   * Browse Problem Set
   * @description ### 權限
   * - System normal (not hidden)
   */
  browse_problem_set_problem_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_problem_set_return'];
        };
      };
    };
  };
  /**
   * Read Problem
   * @description ### 權限
   * - Class manager (hidden)
   * - System normal (not hidden)
   */
  read_problem_problem__problem_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_problem_return'];
        };
      };
    };
  };
  /**
   * Delete Problem
   * @description ### 權限
   * - Class manager
   */
  delete_problem_problem__problem_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_problem_return'];
        };
      };
    };
  };
  /**
   * Edit Problem
   * @description ### 權限
   * - Class manager
   */
  edit_problem_problem__problem_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditProblemInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_problem_return'];
        };
      };
    };
  };
  /**
   * Browse All Testcase Under Problem
   * @description ### 權限
   * - System normal (data without file uuid)
   * - CM (all data)
   */
  browse_all_testcase_under_problem_problem__problem_id__testcase_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_testcase_under_problem_return'];
        };
      };
    };
  };
  /**
   * Add Testcase Under Problem
   * @description ### 權限
   * - Class manager
   */
  add_testcase_under_problem_problem__problem_id__testcase_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddTestcaseInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_testcase_under_problem_return'];
        };
      };
    };
  };
  /**
   * Browse All Assisting Data Under Problem
   * @description ### 權限
   * - class manager
   */
  browse_all_assisting_data_under_problem_problem__problem_id__assisting_data_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_assisting_data_under_problem_return'];
        };
      };
    };
  };
  /**
   * Add Assisting Data Under Problem
   * @description ### 權限
   * - class manager
   */
  add_assisting_data_under_problem_problem__problem_id__assisting_data_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_add_assisting_data_under_problem_problem__problem_id__assisting_data_post'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_assisting_data_under_problem_return'];
        };
      };
    };
  };
  /**
   * Download All Assisting Data
   * @description ### 權限
   * - class manager
   */
  download_all_assisting_data_problem__problem_id__all_assisting_data_post: {
    parameters: {
      query: {
        as_attachment: boolean;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['download_all_assisting_data_return'];
        };
      };
    };
  };
  /**
   * Download All Sample Testcase
   * @description ### 權限
   * - class manager
   */
  download_all_sample_testcase_problem__problem_id__all_sample_testcase_post: {
    parameters: {
      query: {
        as_attachment: boolean;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['download_all_sample_testcase_return'];
        };
      };
    };
  };
  /**
   * Download All Non Sample Testcase
   * @description ### 權限
   * - class manager
   */
  download_all_non_sample_testcase_problem__problem_id__all_non_sample_testcase_post: {
    parameters: {
      query: {
        as_attachment: boolean;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['download_all_non_sample_testcase_return'];
        };
      };
    };
  };
  /**
   * Get Score By Challenge Type Under Problem
   * @description ### 權限
   * - Self
   */
  get_score_by_challenge_type_under_problem_problem__problem_id__score_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_score_by_challenge_type_under_problem_return'];
        };
      };
    };
  };
  /**
   * Get Score By Best Under Problem
   * @description ### 權限
   * - Self
   */
  get_score_by_best_under_problem_problem__problem_id__best_score_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_score_by_best_under_problem_return'];
        };
      };
    };
  };
  /**
   * Rejudge Problem
   * @description ### 權限
   * - Class manager
   */
  rejudge_problem_problem__problem_id__rejudge_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['rejudge_problem_return'];
        };
      };
    };
  };
  /**
   * Get Problem Statistics
   * @description ### 權限
   * - System normal
   */
  get_problem_statistics_problem__problem_id__statistics_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['get_problem_statistics_return'];
        };
      };
    };
  };
  /**
   * Read Testcase
   * @description ### 權限
   * - System normal
   */
  read_testcase_testcase__testcase_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        testcase_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_testcase_return'];
        };
      };
    };
  };
  /**
   * Delete Testcase
   * @description ### 權限
   * - Class manager
   */
  delete_testcase_testcase__testcase_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        testcase_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_testcase_return'];
        };
      };
    };
  };
  /**
   * Edit Testcase
   * @description ### 權限
   * - Class manager
   */
  edit_testcase_testcase__testcase_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        testcase_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditTestcaseInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_testcase_return'];
        };
      };
    };
  };
  /**
   * Upload Testcase Input Data
   * @description ### 權限
   * - Class manager
   */
  upload_testcase_input_data_testcase__testcase_id__input_data_put: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        testcase_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_upload_testcase_input_data_testcase__testcase_id__input_data_put'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['upload_testcase_input_data_return'];
        };
      };
    };
  };
  /**
   * Delete Testcase Input Data
   * @description ### 權限
   * - Class manager
   */
  delete_testcase_input_data_testcase__testcase_id__input_data_delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        testcase_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_testcase_input_data_return'];
        };
      };
    };
  };
  /**
   * Upload Testcase Output Data
   * @description ### 權限
   * - Class manager
   */
  upload_testcase_output_data_testcase__testcase_id__output_data_put: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        testcase_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_upload_testcase_output_data_testcase__testcase_id__output_data_put'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['upload_testcase_output_data_return'];
        };
      };
    };
  };
  /**
   * Delete Testcase Output Data
   * @description ### 權限
   * - Class manager
   */
  delete_testcase_output_data_testcase__testcase_id__output_data_delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        testcase_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_testcase_output_data_return'];
        };
      };
    };
  };
  /**
   * Read Assisting Data
   * @description ### 權限
   * - class manager
   */
  read_assisting_data_assisting_data__assisting_data_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        assisting_data_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_assisting_data_return'];
        };
      };
    };
  };
  /**
   * Edit Assisting Data
   * @description ### 權限
   * - class manager
   */
  edit_assisting_data_assisting_data__assisting_data_id__put: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        assisting_data_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_edit_assisting_data_assisting_data__assisting_data_id__put'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_assisting_data_return'];
        };
      };
    };
  };
  /**
   * Delete Assisting Data
   * @description ### 權限
   * - class manager
   */
  delete_assisting_data_assisting_data__assisting_data_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        assisting_data_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_assisting_data_return'];
        };
      };
    };
  };
  /**
   * Browse All Submission Language
   * @description ### 權限
   * - System normal
   */
  browse_all_submission_language_submission_language_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_submission_language_return'];
        };
      };
    };
  };
  /**
   * Add Submission Language
   * @description ### 權限
   * - System manager
   */
  add_submission_language_submission_language_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddSubmissionLanguageInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_submission_language_return'];
        };
      };
    };
  };
  /**
   * Edit Submission Language
   * @description ### 權限
   * - System manager
   */
  edit_submission_language_submission_language__language_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        language_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditSubmissionLanguageInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_submission_language_return'];
        };
      };
    };
  };
  /**
   * Submit
   * @description ### 權限
   * - System Manager (all)
   * - System normal (non scheduled)
   *
   * ### 限制
   * - 上傳檔案 < 1mb
   */
  submit_problem__problem_id__submission_post: {
    parameters: {
      query: {
        language_id: number;
      };
      header: {
        'auth-token'?: string;
        'content-length': number;
      };
      path: {
        problem_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_submit_problem__problem_id__submission_post'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['submit_return'];
        };
      };
    };
  };
  /**
   * Browse Submission
   * @description ### 權限
   * - Self: see self
   *
   * ### Available columns
   * {'problem_id': 'int', 'language_id': 'int', 'submit_time': 'ServerTZDatetime'}
   */
  browse_submission_submission_get: {
    parameters: {
      query: {
        account_id: number;
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_submission_return'];
        };
      };
    };
  };
  /**
   * Batch Get Submission Judgment
   * @description ### 權限
   * - System Normal
   *
   * ### Notes
   * - `submission_ids`: list of int
   */
  batch_get_submission_judgment_submission_judgment_batch_get: {
    parameters: {
      query: {
        submission_ids: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['batch_get_submission_judgment_return'];
        };
      };
    };
  };
  /**
   * Read Submission
   * @description ### 權限
   * - Self
   * - Class manager
   */
  read_submission_submission__submission_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        submission_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_submission_return'];
        };
      };
    };
  };
  /**
   * Browse All Submission Judgment
   * @description ### 權限
   * - Class manager
   */
  browse_all_submission_judgment_submission__submission_id__judgment_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        submission_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_submission_judgment_return'];
        };
      };
    };
  };
  /**
   * Read Submission Latest Judgment
   * @description ### 權限
   * - Self: see self
   * - Class manager: see class
   */
  read_submission_latest_judgment_submission__submission_id__latest_judgment_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        submission_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_submission_latest_judgment_return'];
        };
      };
    };
  };
  /**
   * Rejudge Submission
   * @description ### 權限
   * - Class manager
   */
  rejudge_submission_submission__submission_id__rejudge_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        submission_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['rejudge_submission_return'];
        };
      };
    };
  };
  /**
   * Browse All Judgment Verdict
   * @description ### 權限
   * - Public
   */
  browse_all_judgment_verdict_judgment_verdict_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_judgment_verdict_return'];
        };
      };
    };
  };
  /**
   * Read Judgment
   * @description ### 權限
   * - Self
   * - Class manager
   */
  read_judgment_judgment__judgment_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        judgment_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_judgment_return'];
        };
      };
    };
  };
  /**
   * Browse All Judgment Judge Case
   * @description ### 權限
   * - Self
   * - Class manager
   */
  browse_all_judgment_judge_case_judgment__judgment_id__judge_case_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        judgment_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_all_judgment_judge_case_return'];
        };
      };
    };
  };
  /**
   * Read Essay
   * @description ### 權限
   * - class normal
   */
  read_essay_essay__essay_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        essay_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_essay_return'];
        };
      };
    };
  };
  /**
   * Delete Essay
   * @description ### 權限
   * - class manager
   */
  delete_essay_essay__essay_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        essay_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_essay_return'];
        };
      };
    };
  };
  /**
   * Edit Essay
   * @description ### 權限
   * - class manager
   */
  edit_essay_essay__essay_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        essay_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditEssayInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_essay_return'];
        };
      };
    };
  };
  /**
   * Download All Essay Submission
   * @description ### 權限
   * - class manager
   */
  download_all_essay_submission_essay__essay_id__all_essay_submission_post: {
    parameters: {
      query: {
        as_attachment: boolean;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        essay_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['download_all_essay_submission_return'];
        };
      };
    };
  };
  /**
   * Browse Essay Submission By Essay Id
   * @description ### 權限
   * - Self
   *
   * ### Available columns
   * {'id': 'int', 'account_id': 'int', 'essay_id': 'int', 'content_file_uuid': 'UUID', 'filename': 'str', 'submit_time': 'ServerTZDatetime'}
   */
  browse_essay_submission_by_essay_id_essay__essay_id__essay_submission_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        essay_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_essay_submission_by_essay_id_return'];
        };
      };
    };
  };
  /**
   * Upload Essay
   * @description ### 權限
   * - class normal
   *
   * ### 限制
   * - 上傳檔案 < 10mb
   */
  upload_essay_essay__essay_id__essay_submission_post: {
    parameters: {
      header: {
        'auth-token'?: string;
        'content-length': number;
      };
      path: {
        essay_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_upload_essay_essay__essay_id__essay_submission_post'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['upload_essay_return'];
        };
      };
    };
  };
  /**
   * Read Essay Submission
   * @description ### 權限
   * - class manager (always)
   * - class normal (self)
   */
  read_essay_submission_essay_submission__essay_submission_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        essay_submission_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_essay_submission_return'];
        };
      };
    };
  };
  /**
   * Reupload Essay
   * @description ### 權限
   * - Class normal = self
   *
   * ### 限制
   * - 上傳檔案 < 10mb
   */
  reupload_essay_essay_submission__essay_submission_id__put: {
    parameters: {
      header: {
        'auth-token'?: string;
        'content-length': number;
      };
      path: {
        essay_submission_id: number;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_reupload_essay_essay_submission__essay_submission_id__put'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['reupload_essay_return'];
        };
      };
    };
  };
  /**
   * Read Peer Review
   * @description ### 權限
   * - Class manager (hidden)
   * - Class normal (not hidden)
   */
  read_peer_review_peer_review__peer_review_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_peer_review_return'];
        };
      };
    };
  };
  /**
   * Delete Peer Review
   * @description ### 權限
   * - Class manager
   */
  delete_peer_review_peer_review__peer_review_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_peer_review_return'];
        };
      };
    };
  };
  /**
   * Edit Peer Review
   * @description ### 權限
   * - Class manager
   */
  edit_peer_review_peer_review__peer_review_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditPeerReviewInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_peer_review_return'];
        };
      };
    };
  };
  /**
   * Browse Peer Review Record
   * @description ### 權限
   * - Class manager (full)
   * - Self (看不到對方)
   *
   * ### Available columns
   * {'grader_id': 'int', 'receiver_id': 'int', 'score': 'int', 'comment': 'str', 'submit_time': 'ServerTZDatetime'}
   */
  browse_peer_review_record_peer_review__peer_review_id__record_get: {
    parameters: {
      query: {
        limit: number;
        offset: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_peer_review_record_return'];
        };
      };
    };
  };
  /**
   * Assign Peer Review Record
   * @description 發互評 (決定 A 要評誰 )
   *
   * ### 權限
   * - Self is class *normal ONLY*
   */
  assign_peer_review_record_peer_review__peer_review_id__record_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['assign_peer_review_record_return'];
        };
      };
    };
  };
  /**
   * Read Peer Review Record
   * @description ### 權限
   * - Class manager (full)
   * - Self = Class normal (看不到對方)
   */
  read_peer_review_record_peer_review_record__peer_review_record_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_record_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_peer_review_record_return'];
        };
      };
    };
  };
  /**
   * Submit Peer Review Record
   * @description 互評完了，交互評成績評語
   *
   * ### 權限
   * - Self is class *normal ONLY*
   */
  submit_peer_review_record_peer_review_record__peer_review_record_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_record_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['SubmitPeerReviewInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['submit_peer_review_record_return'];
        };
      };
    };
  };
  /**
   * Browse Account Received Peer Review Record
   * @description ### 權限
   * - Class Manager (all)
   * - Self = Class normal
   */
  browse_account_received_peer_review_record_peer_review__peer_review_id__account__account_id__receive_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_account_received_peer_review_record_return'];
        };
      };
    };
  };
  /**
   * Browse Account Reviewed Peer Review Record
   * @description ### 權限
   * - Class Manager (all)
   * - Self = Class normal
   */
  browse_account_reviewed_peer_review_record_peer_review__peer_review_id__account__account_id__review_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
        account_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_account_reviewed_peer_review_record_return'];
        };
      };
    };
  };
  /**
   * Read Scoreboard
   * @description ### 權限
   * - Class normal
   */
  read_scoreboard_scoreboard__scoreboard_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        scoreboard_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_scoreboard_return'];
        };
      };
    };
  };
  /**
   * Delete Scoreboard
   * @description ### 權限
   * - Class manager
   */
  delete_scoreboard_scoreboard__scoreboard_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        scoreboard_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_scoreboard_return'];
        };
      };
    };
  };
  /**
   * View Team Project Scoreboard
   * @description ### 權限
   * - Class normal
   */
  view_team_project_scoreboard_team_project_scoreboard_view__scoreboard_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        scoreboard_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['view_team_project_scoreboard_return'];
        };
      };
    };
  };
  /**
   * Edit Team Project Scoreboard
   * @description ### 權限
   * - Class manager
   */
  edit_team_project_scoreboard_team_project_scoreboard__scoreboard_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        scoreboard_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['processor__http__scoreboard_setting_team_project__EditScoreboardInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_team_project_scoreboard_return'];
        };
      };
    };
  };
  /**
   * View Team Contest Scoreboard
   * @description ### 權限
   * - System Normal
   */
  view_team_contest_scoreboard_team_contest_scoreboard_view__scoreboard_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        scoreboard_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['view_team_contest_scoreboard_return'];
        };
      };
    };
  };
  /**
   * Edit Team Contest Scoreboard
   * @description ### 權限
   * - Class manager
   */
  edit_team_contest_scoreboard_team_contest_scoreboard__scoreboard_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        scoreboard_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['processor__http__scoreboard_setting_team_contest__EditScoreboardInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_team_contest_scoreboard_return'];
        };
      };
    };
  };
  /**
   * Browse Announcement
   * @description ### 權限
   * - System manager (all)
   * - System guest (limited)
   *
   * ### Available columns
   * {'id': 'int', 'title': 'str', 'content': 'str', 'author_id': 'int', 'post_time': 'ServerTZDatetime', 'expire_time': 'ServerTZDatetime', 'is_deleted': 'bool'}
   */
  browse_announcement_announcement_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_announcement_return'];
        };
      };
    };
  };
  /**
   * Add Announcement
   * @description ### 權限
   * - System manager
   */
  add_announcement_announcement_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddAnnouncementInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_announcement_return'];
        };
      };
    };
  };
  /**
   * Read Announcement
   * @description ### 權限
   * - System manager (all)
   * - System guest (limited)
   */
  read_announcement_announcement__announcement_id__get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        announcement_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['read_announcement_return'];
        };
      };
    };
  };
  /**
   * Delete Announcement
   * @description ### 權限
   * - System manager
   */
  delete_announcement_announcement__announcement_id__delete: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        announcement_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['delete_announcement_return'];
        };
      };
    };
  };
  /**
   * Edit Announcement
   * @description ### 權限
   * - System manager
   */
  edit_announcement_announcement__announcement_id__patch: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        announcement_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditAnnouncementInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_announcement_return'];
        };
      };
    };
  };
  /**
   * Browse Access Log
   * @description ### 權限
   * - Class+ manager
   *
   * ### Available columns
   * {'access_time': 'ServerTZDatetime', 'request_method': 'str', 'resource_path': 'str', 'ip': 'str', 'account_id': 'int'}
   */
  browse_access_log_access_log_get: {
    parameters: {
      query: {
        limit: number;
        offset: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_access_log_return'];
        };
      };
    };
  };
  /**
   * Browse Account With Default Student Id
   * @description ### 權限
   * - System Manager
   *
   * ### Available columns
   * {'account_id': 'int', 'username': 'str', 'real_name': 'str', 'student_id': 'str'}
   */
  browse_account_with_default_student_id_view_account_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_account_with_default_student_id_return'];
        };
      };
    };
  };
  /**
   * Browse Class Member
   * @description ### 權限
   * - Class normal
   * - Class+ manager
   *
   * ### Available columns
   * {'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'abbreviated_name': 'str', 'role': 'RoleType', 'class_id': 'int'}
   */
  browse_class_member_class__class_id__view_member_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_class_member_return'];
        };
      };
    };
  };
  /**
   * Browse Submission Under Class
   * @description ### 權限
   * - Class manager
   *
   * ### Available columns
   * {'submission_id': 'int', 'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'challenge_id': 'int', 'challenge_title': 'str', 'problem_id': 'int', 'challenge_label': 'str', 'verdict': 'VerdictType', 'submit_time': 'ServerTZDatetime', 'class_id': 'int'}
   */
  browse_submission_under_class_class__class_id__view_submission_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_submission_under_class_return'];
        };
      };
    };
  };
  /**
   * Browse Submission
   * @description ### 權限
   * - Self: see self
   *
   * ### Available columns
   * {'submission_id': 'int', 'course_id': 'int', 'course_name': 'str', 'class_id': 'int', 'class_name': 'str', 'challenge_id': 'int', 'challenge_title': 'str', 'problem_id': 'int', 'challenge_label': 'str', 'verdict': 'VerdictType', 'submit_time': 'ServerTZDatetime', 'account_id': 'int'}
   */
  browse_submission_view_my_submission_get: {
    parameters: {
      query: {
        account_id: number;
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_submission_return'];
        };
      };
    };
  };
  /**
   * Browse My Submission Under Problem
   * @description ### 權限
   * - Self: see self
   *
   * ### Available columns
   * {'submission_id': 'int', 'judgment_id': 'int', 'verdict': 'VerdictType', 'score': 'int', 'total_time': 'int', 'max_memory': 'int', 'submit_time': 'ServerTZDatetime', 'account_id': 'int', 'problem_id': 'int'}
   */
  browse_my_submission_under_problem_problem__problem_id__view_my_submission_get: {
    parameters: {
      query: {
        account_id: number;
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        problem_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_my_submission_under_problem_return'];
        };
      };
    };
  };
  /**
   * Browse Problem Set Under Class
   * @description ### 權限
   * - System normal (not hidden)
   *
   * ### Available columns
   * {'challenge_id': 'int', 'challenge_title': 'str', 'problem_id': 'int', 'challenge_label': 'str', 'problem_title': 'str', 'class_id': 'int'}
   */
  browse_problem_set_under_class_class__class_id__view_problem_set_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['browse_problem_set_under_class_return'];
        };
      };
    };
  };
  /**
   * Browse Class Grade
   * @description ### 權限
   * - Class manager (all)
   * - Class normal (self)
   *
   * ### Available columns
   * {'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'title': 'str', 'score': 'str', 'update_time': 'ServerTZDatetime', 'grade_id': 'int', 'class_id': 'int'}
   */
  browse_class_grade_class__class_id__view_grade_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        class_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_class_grade_return'];
        };
      };
    };
  };
  /**
   * Browse Access Log
   * @description ### 權限
   * - Class+ manager
   *
   * ### Available columns
   * {'account_id': 'int', 'username': 'str', 'student_id': 'str', 'real_name': 'str', 'ip': 'str', 'resource_path': 'str', 'request_method': 'str', 'access_time': 'ServerTZDatetime', 'access_log_id': 'int'}
   */
  browse_access_log_view_access_log_get: {
    parameters: {
      query: {
        limit: number;
        offset: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['pydantic__dataclasses__browse_access_log_return'];
        };
      };
    };
  };
  /**
   * Peer Review Summary Review
   * @description ### 權限
   * - Class Manager
   *
   * ### Available columns
   * {'username': 'str', 'real_name': 'str', 'student_id': 'str', 'average_score': 'float'}
   */
  peer_review_summary_review_peer_review__peer_review_id__view_reviewer_summary_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['peer_review_summary_review_return'];
        };
      };
    };
  };
  /**
   * Peer Review Summary Receive
   * @description ### 權限
   * - Class Manager
   *
   * ### Available columns
   * {'username': 'str', 'real_name': 'str', 'student_id': 'str', 'average_score': 'float'}
   */
  peer_review_summary_receive_peer_review__peer_review_id__view_receiver_summary_get: {
    parameters: {
      query?: {
        limit?: number;
        offset?: number;
        filter?: string;
        sort?: string;
      };
      header?: {
        'auth-token'?: string;
      };
      path: {
        peer_review_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['peer_review_summary_receive_return'];
        };
      };
    };
  };
  /**
   * View Team Contest Scoreboard Runs
   * @description ### 權限
   * - System Normal
   */
  view_team_contest_scoreboard_runs_hardcode_team_contest_scoreboard__scoreboard_id__runs_get: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        scoreboard_id: number;
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['view_team_contest_scoreboard_runs_return'];
        };
      };
    };
  };
  /** Login */
  login_account_jwt_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['LoginInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['login_return'];
        };
      };
    };
  };
  /**
   * Add Normal Account
   * @description ### 權限
   * - System Manager
   */
  add_normal_account_account_normal_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['AddNormalAccountInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['add_normal_account_return'];
        };
      };
    };
  };
  /**
   * Import Account
   * @description ### 權限
   * - System Manager
   */
  import_account_account_import_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'multipart/form-data': components['schemas']['Body_import_account_account_import_post'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['import_account_return'];
        };
      };
    };
  };
  /**
   * Edit Password
   * @description ### 權限
   * - System Manager
   * - Self (need old password)
   */
  edit_password_account__account_id__pass_hash_put: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
      path: {
        account_id: number;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['EditPasswordInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['edit_password_return'];
        };
      };
    };
  };
  /** Reset Password */
  reset_password_account_reset_password_post: {
    parameters: {
      header?: {
        'auth-token'?: string;
      };
    };
    requestBody: {
      content: {
        'application/json': components['schemas']['ResetPasswordInput'];
      };
    };
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': components['schemas']['reset_password_return'];
        };
      };
    };
  };
  /**
   * Metrics
   * @description Endpoint that serves Prometheus metrics.
   */
  metrics_metrics_get: {
    responses: {
      /** @description Successful Response */
      200: {
        content: {
          'application/json': unknown;
        };
      };
    };
  };
}
